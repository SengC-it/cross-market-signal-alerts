...
const REVIEW_RECOVERY_LIMIT = Math.min(20, Math.max(1, Number(process.env.FUNDING_CARRY_V2_REVIEW_LIMIT || 12)));
const REVIEW_STALE_GRACE_HOURS = 6;
const FOUR_HOUR_REVIEW_LOOKBACK = 140;
...
export async function runFundingCarryV2PaperScan({ dryRun = false, now = Date.now() } = {}) {
  ...
  const recovery = dryRun
    ? emptyRecoveryResult()
    : await monitorOpenFundingCarryV2Runs({ now, limit: REVIEW_RECOVERY_LIMIT });
  ...
  if (existingRun) {
    ...
    return buildScanResult({ run: existingRun, review, email, recovery, status: "already_recorded", startedAt });
  }
  ...
  if (dryRun) return buildScanResult({ run: row, review: row.review, email: { emailStatus: "dry_run" }, recovery, status: "dry_run", startedAt });
  ...
  return buildScanResult({ run: row, review: row.review, email, recovery, status: "recorded", startedAt });
}

export async function monitorOpenFundingCarryV2Runs({
  now = Date.now(),
  limit = REVIEW_RECOVERY_LIMIT,
  fetchRuns = fetchPendingPaperModelRunsForModel,
  reviewRun = tryReviewFundingCarryV2PaperRun,
  persistReview = updatePendingPaperModelReview
} = {}) {
  const runs = await fetchRuns({
    modelId: FUNDING_CARRY_V2_MODEL.id,
    limit: Math.min(20, Math.max(1, Number(limit) || REVIEW_RECOVERY_LIMIT))
  });
  const eligibleRuns = (runs || [])
    .filter((run) => run?.model_id === FUNDING_CARRY_V2_MODEL.id)
    .filter((run) => run?.review?.status === "pending")
    .filter((run) => Array.isArray(run?.targets) && run.targets.length)
    .sort((left, right) => new Date(left.rebalance_time).getTime() - new Date(right.rebalance_time).getTime());
  const results = await mapLimit(eligibleRuns, 4, async (run) => {
    try {
      const review = await reviewRun({ run, now, persistReview });
      return {
        rebalanceTime: run.rebalance_time,
        status: review?.status || "pending",
        stale: review?.diagnostics?.status === "stale_review",
        reason: review?.reason || null
      };
    } catch (error) {
      const reason = error instanceof Error ? error.message : String(error);
      const review = pendingReviewWithDiagnostics({ run, now, reason });
      try {
  ...
  return {
    ...
    failed: results.filter((result) => result.status === "retry_failed").length,
    stale: results.filter((result) => result.stale).length,
    results
  };
}

export function isStaleFundingCarryV2Run(run, now = Date.now(), graceHours = REVIEW_STALE_GRACE_HOURS) {
  if (run?.model_id !== FUNDING_CARRY_V2_MODEL.id || run?.review?.status !== "pending") return false;
  const entryTime = new Date(run.rebalance_time).getTime();
  if (!Number.isFinite(entryTime)) return false;
  const holdingHours = Math.max(
    FUNDING_CARRY_V2_MODEL.maxHoldingHours,
    ...(Array.isArray(run.targets) ? run.targets : []).map((target) => Number(target.maxHoldingHours || 0)).filter(Number.isFinite)
  );
  return Number(now) > entryTime + (holdingHours + Number(graceHours || 0)) * HOUR_MS;
}
...
export function renderFundingCarryV2PaperEmail(run, { performanceSnapshot = null } = {}) {
  const targets = Array.isArray(run?.targets) ? run.targets : [];
...
function reviewPosition({ target, entryTime, hourlyCandles, fourHourlyCandles, fundingRows, now }) {
  ...
  const tradingCost = Number.isFinite(Number(target.modeledRoundTripCostPct))
    ? Number(target.modeledRoundTripCostPct)
    : ROUND_TRIP_COST;
...
export async function tryReviewFundingCarryV2PaperRun({
  run,
  now,
  getCandles = getFuturesCandlesRange,
  getFunding = getFuturesFundingHistory,
  persistReview = updatePendingPaperModelReview
}) {
  ...
  await mapLimit(run.targets, 4, async (target) => {
    const maxExitTime = entryTime + Number(target.maxHoldingHours || FUNDING_CARRY_V2_MODEL.maxHoldingHours) * HOUR_MS;
    const dataEndTime = Math.min(Number(now), maxExitTime);
    const [oneHour, fourHour, rates] = await Promise.all([
      getCandles(target.symbol, "1h", entryTime, dataEndTime, 72),
      getCandles(target.symbol, "4h", entryTime - FOUR_HOUR_REVIEW_LOOKBACK * FOUR_HOUR_MS, dataEndTime, FOUR_HOUR_REVIEW_LOOKBACK + 20),
      getFunding(target.symbol, entryTime - 280 * FUNDING_INTERVAL_MS, Math.min(Number(now), maxExitTime))
    ]);
    hourly.set(target.symbol, oneHour);
    fourHourly.set(target.symbol, fourHour);
    funding.set(target.symbol, rates);
  });
  let review = reviewFundingCarryV2PaperRun({ run, hourlyCandlesBySymbol: hourly, fourHourlyBySymbol: fourHourly, fundingBySymbol: funding, now });
  if (review.status === "pending") review = pendingReviewWithDiagnostics({ run, now, review });
  await persistReview({ modelId: run.model_id, rebalanceTime: run.rebalance_time, review });
  return review;
}
...
async function sendFundingCarryV2PaperEmailIfNeeded({ modelId, rebalanceTime }) {
  ...
  try {
    ...
    const sent = await sendEmail({
      ...renderFundingCarryV2PaperEmail(claimed, { performanceSnapshot }),
      idempotencyKey
    });
...
function pendingReviewWithDiagnostics({ run, now, review = null, reason = null }) {
  const stale = isStaleFundingCarryV2Run(run, now);
  const previousAttempts = Number(run?.review?.diagnostics?.attemptCount || 0);
  const retryReason = reason
    || review?.positions?.find((position) => position.status === "pending")?.reason
    || review?.reason
    || "exit conditions not reached";
  return {
    ...(review || pendingReview(retryReason)),
    status: "pending",
    reason: stale ? `stale_review: ${retryReason}` : retryReason,
    diagnostics: {
      status: stale ? "stale_review" : "retry_pending",
      checkedAt: new Date(now).toISOString(),
      retryReason,
      retryEligible: true,
      attemptCount: previousAttempts + 1,
      maxHoldingHours: FUNDING_CARRY_V2_MODEL.maxHoldingHours,
      gracePeriodHours: REVIEW_STALE_GRACE_HOURS
    }
  };
}
...
function buildScanResult({ run, review, email, recovery = emptyRecoveryResult(), status, startedAt }) {
  return {
    group: "funding-carry-v2-paper",
...
function emptyRecoveryResult() {
  return { checked: 0, reviewed: 0, pending: 0, failed: 0, stale: 0, results: [] };
}
