...
export async function runReviewRecovery({
  ...
  if (!isConfigured()) {
    ...
    reviewRecovery.health = "degraded";
  ...
  while (Date.now() < deadline && (!ordinaryExhausted || !paperExhausted)) {
    ...
    const paperCandidates = paper.filter((run) =>
      Array.isArray(run?.targets)
      && run.targets.length > 0
      && paperModelRunKey(run)
      && !excludedPaperKeys.has(paperModelRunKey(run))
    );
  ...
  reviewRecovery.durationMs = Date.now() - startedAt;
  reviewRecovery.hasMore = {
    ordinary: !ordinaryExhausted,
    paper: !paperExhausted
  };
  reviewRecovery.health = resolveRecoveryHealth(reviewRecovery, errors);
  if (!dryRun) {
    try {
      await writeRunLog({
...
export async function processOrdinaryReviewAlert(alert, {
  ...
  loadLowerCandles = fetchReviewLowerCandles,
  ...
  let failed = false;
  try {
    if (payload.kind === "futures_arbitrage") {
      nextReview = reviewArbitrageAlert(alert, now);
    } else {
      const loaded = normalizeReviewCandleLoad(await loadCandles(alert, now));
      nextReview = review(alert, loaded.candles, now);
      if (requiresLowerTimeframeReplay(nextReview)) {
        const lowerTimeframeCandles = loaded.lowerTimeframeCandles
          || (loadCandles === fetchReviewCandles || loadLowerCandles !== fetchReviewLowerCandles
            ? await loadLowerCandles(alert, now)
            : []);
        nextReview = review(alert, loaded.candles, now, {
          lowerTimeframe: "1m",
...
async function fetchReviewCandles(alert, now = Date.now()) {
  ...
  const maxHoldingTime = finiteReviewNumber(tradeSpec.maxHoldingTime);
  ...
  const endTime = Number.isFinite(maxHoldingTime)
    ...
    : Number(now);
  if (isFuturesReviewMarket(alert)) {
...
async function fetchReviewLowerCandles(alert, now = Date.now()) {
  const tradeSpec = getTradeSpecForAlert(alert);
  if (!tradeSpec) return [];
  const payload = alert?.payload || {};
  const sentAt = parseReviewTimestamp(
    alert?.sent_at
      ?? alert?.sentAt
      ?? payload.sentAt
      ?? payload.sent_at
  ...
  const lowerStartSource = [sentAt, entryEligibleAt].filter(Number.isFinite).length
    ? Math.max(...[sentAt, entryEligibleAt].filter(Number.isFinite))
    : null;
  if (!Number.isFinite(lowerStartSource)) return [];

  const lowerStart = ceilToMinute(lowerStartSource);
  const maxHoldingTime = finiteReviewNumber(tradeSpec.maxHoldingTime);
  const replayEnd = Number.isFinite(maxHoldingTime)
    ? Math.min(Number(now), maxHoldingTime)
    : Number(now);
  const lowerEnd = replayEnd + MINUTE_MS;
  if (!Number.isFinite(lowerEnd) || lowerEnd <= lowerStart) return [];

  const futures = isFuturesReviewMarket(alert);
  const requestedMinutes = Math.max(1, Math.ceil((lowerEnd - lowerStart) / MINUTE_MS) + 2);
  const limit = Math.min(futures ? 1500 : 1000, requestedMinutes);
  if (futures) {
    return getFuturesCandlesRange(alert.asset, "1m", lowerStart, lowerEnd, limit);
  }
  return getCryptoCandlesRange(alert.asset, "1m", lowerStart, lowerEnd, limit);
}

function normalizeReviewCandleLoad(loaded) {
  if (Array.isArray(loaded)) {
    return { candles: loaded, lowerTimeframeCandles: null };
  }
  return {
    candles: Array.isArray(loaded?.candles) ? loaded.candles : [],
    lowerTimeframeCandles: Array.isArray(loaded?.lowerTimeframeCandles)
      ? loaded.lowerTimeframeCandles
      : null
  };
}

function requiresLowerTimeframeReplay(review) {
  if (review?.status !== "pending") return false;
  return [
    "pending_partial_candle",
    "ambiguous_intrabar",
    "time_stop_requires_lower_timeframe"
  ].includes(String(review.reason || ""))
    || review.state === "ambiguous"
    || review.dataQuality === "INCOMPLETE_INTRABAR_DATA"
    || review.intrabarQuality === "INCOMPLETE_INTRABAR_DATA";
}

function isFuturesReviewMarket(alert) {
  const market = String(alert?.payload?.market || "");
  return market.includes("USDT") || market.includes("合约") || market.includes("鍚堢害");
}

function ceilToMinute(value) {
  return Math.ceil(Number(value) / MINUTE_MS) * MINUTE_MS;
}
...
function finiteReviewNumber(value) {
...
function resolveRecoveryHealth(reviewRecovery, errors) {
  if (errors.length
    || reviewRecovery.ordinarySignals.failed > 0
    || reviewRecovery.paperRuns.failed > 0) {
    return "degraded";
  }
  if (reviewRecovery.hasMore.ordinary || reviewRecovery.hasMore.paper) return "backlog";
  return "healthy";
}
...
export function safeErrorText(error) {
  ...
  return text
    ...
    .replace(/(supabase[_-]?(service[_-])?role[_-]?key|cron[_-]?secret|dashboard[_-]?secret|api[-_ ]?key|authorization|secret|password)\s*[:=]?\s*(?:Bearer\s+)?\S+/gi, "$1=[redacted]")
    .slice(0, 500);
}
