...
export function reviewV31PaperRun({
  ...
  if (!Number.isFinite(entryTime) || Number(exitTime) < expectedExitTime) {
    return {
      status: "pending",
      reason: "持仓周期未结束"
    };
  ...
  const positions = targets.map((target) => {
    ...
    const exitCandle = Array.isArray(candles)
      ? candles.find((candle) => Number(candle.openTime) === expectedExitTime)
      : null;
    const exitPrice = Number(exitCandle?.open);
    if (!(entryPrice > 0) || !(exitPrice > 0) || !Number.isFinite(weight)) {
      throw new Error(`V3.1 PAPER review is missing an exit price: ${symbol}`);
    }
    ...
    const fundingRate = fundingRows
      .filter((item) => Number(item.fundingTime) > entryTime && Number(item.fundingTime) <= expectedExitTime)
    ...
    const positionReturn = directionalPriceReturn + positionFundingReturn - V31_MODEL.roundTripCostRate;
    ...
    return {
      ...
      tradingCost: V31_MODEL.roundTripCostRate,
  ...
  const tradingCost = V31_MODEL.roundTripCostRate * grossExposure;
