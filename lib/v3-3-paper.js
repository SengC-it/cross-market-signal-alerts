...
export function renderV33PaperEmail(run) {
  ...
  const subject = `[SHADOW PAPER] V3.3 新信号：${longs.length}多+${shorts.length}空 | ${date}`;
  const lines = [
    "V3.3 组合波动率目标 + 灾难止损 + 回撤熔断模型出现新的周调仓信号。",
    "",
    "重要：这是 SHADOW PAPER 前向验证提醒，不是实盘下单指令。",
    `模型版本：${run.model_version || V33_MODEL.version}`,
    `实盘部署门槛：${run.deployment_gate_passed ? "已通过" : "未通过"}`,
    `实盘资金权重：${formatPercent(Number(run.capital_weight || 0))}`,
    `调仓时间：${rebalanceTime}`,
    `30日预测年化波动率：${formatPercent(risk.forecastAnnualVolatility)}`,
    `目标年化波动率：${formatPercent(risk.targetAnnualVolatility)}`,
    `组合总敞口：${formatNumber(run.gross_exposure, 4)}x`,
    `组合灾难止损：净收益触及 -${formatPercent(risk.catastropheStop)}，下一小时开盘纸面退出`,
    `账户回撤熔断：-${formatPercent(risk.breakerDrawdown)}，随后 ${risk.breakerCooldownWeeks} 周保持现金`,
    "止盈：无固定止盈；最长持仓 168 小时。",
    "",
    "做多目标：",
...
function formatPaperTarget(target) {
  ...
  return `- ${target.symbol} | 权重 ${weight} | 参考价 ${reference} | 残差分数 ${score}`;
}
