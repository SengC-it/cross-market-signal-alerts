...
export function renderV33PaperEmail(run) {
  ...
  const isUnifiedV34 = modelVersion.startsWith("V3.4");
  ...
  const currentPnl = snapshot.currentReturnPct == null
    ? "等待首个完整小时收盘"
    : `${formatSignedPercent(snapshot.currentReturnPct)} / ${formatSignedUsdt(snapshot.currentPnlUsdt)}`;
  const currentPnlTime = snapshot.markTime
    ? `，截至 ${formatBeijingTime(snapshot.markTime)}`
    : "";
  const remainingToStop = snapshot.remainingToStopPct == null
    ? "等待首个完整小时收盘"
    : `${formatPercent(snapshot.remainingToStopPct)} / ${formatUsdt(snapshot.remainingToStopUsdt)}`;
  const lines = [
    "这是一封模拟交易提醒，不会自动下单。",
    "",
    ...
    `标准化模拟本金：${formatUsdt(snapshot.reportingNotionalUsdt)}`,
    `最新组合浮动盈亏：${currentPnl}${currentPnlTime}`,
    `组合灾难止损：${formatSignedPercent(-snapshot.catastropheStopPct)} / ${formatSignedUsdt(snapshot.catastropheStopAmountUsdt)}`,
    `距离组合止损：${remainingToStop}`,
    `最晚到期时间：${formatBeijingTime(snapshot.expiresAt)}`,
    ...
    "- 不设置固定盈利目标，最多观察 7 天。",
    `- ${isUnifiedV34 ? "绩效安全闸：" : ""}如果模拟账户比之前最高点少了 ${formatPercent(risk.breakerDrawdown)}，暂停建立新仓位 ${risk.breakerCooldownWeeks} 周。`,
    "",
