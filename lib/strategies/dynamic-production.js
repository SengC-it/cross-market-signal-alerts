...
export function benchmarkMomentum24hAsOf({
  ...
  const visible = candles
    ...
    .filter(({ candle }) => candleCloseTime(candle, interval) <= timestamp);
  if (!Number.isFinite(timestamp) || visible.length < 2) return null;
  const latest = visible.at(-1);
  const lookback = Math.max(1, Math.round(24 / Math.max(0.25, intervalHours(interval))));
  const previous = visible[Math.max(0, visible.length - 1 - lookback)];
  const latestClose = Number(latest.candle?.close);
  const previousClose = Number(previous.candle?.close);
