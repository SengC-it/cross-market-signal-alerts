...
export function resolveIntrabarExit({
  ...
  const lower = hasLowerInput
    ...
    : null;
  if (lower?.complete) {
    for (let index = 0; index < lower.candles.length; index++) {
      const candle = lower.candles[index];
...
export function resolveTimeStop({
  ...
  const lower = hasLowerInput
    ? inspectLowerTimeframe({
      candles: lowerTimeframeCandles,
      startTime,
      endTime: baseCloseTime,
      lowerTimeframe
    })
    : null;
  if (insideBaseCandle && lower?.complete) {
    const stopCandleIndex = lower.candles.findIndex((candle) => Number(candle.openTime) >= maxHoldingTime);
    const containingIndex = stopCandleIndex >= 0
      ? stopCandleIndex
      : lower.candles.findIndex((candle) => Number(candle.openTime) + lower.intervalMs > maxHoldingTime);
    const stopCandle = lower.candles[containingIndex];
    if (stopCandle) {
      const stopOpen = Number(stopCandle.openTime);
      const exactOpen = stopOpen >= maxHoldingTime;
      return {
        resolved: true,
        exitReason: "time_stop",
        exitTime: exactOpen ? stopOpen : stopOpen + lower.intervalMs,
        exitMarketPrice: exactOpen ? Number(stopCandle.open) : Number(stopCandle.close),
        resolution: lower.resolution,
        dataQuality: lower.resolution,
        replayCandles: lower.candles,
        replayCandleIndex: containingIndex,
        lowerTimeframeReplayed: true,
...
function inspectLowerTimeframe({ candles, startTime, endTime, lowerTimeframe }) {
  ...
  const firstOpen = Number(selected[0]?.openTime);
  const lastOpen = Number(selected.at(-1)?.openTime);
  const hasRows = selected.length > 0;
  let complete = validInterval && hasRows && firstOpen === startTime && lastOpen + intervalMs >= endTime;
  for (let index = 1; complete && index < selected.length; index++) {
    const gap = Number(selected[index].openTime) - Number(selected[index - 1].openTime);
    if (gap !== intervalMs) complete = false;
  }
  ...
  return {
    ...
    coverage: {
      ...
      coverageEnd: hasRows && validInterval ? lastOpen + intervalMs : null,
      eventCount: selected.length,
      complete,
      gaps: complete ? [] : [{ start: firstOpen || startTime, end: endTime, reason: "LOWER_TF_COVERAGE_INCOMPLETE" }],
