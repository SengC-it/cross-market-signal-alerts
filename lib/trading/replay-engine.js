...
function inspectLowerTimeframe({ candles, startTime, endTime, lowerTimeframe }) {
  ...
  const safeCandles = selected.filter((candle) => Number(candle.openTime) + intervalMs <= endTime);
  ...
  let contiguous = validInterval && hasRows && firstOpen === startTime;
  for (let index = 1; contiguous && index < safeCandles.length; index++) {
    const gap = Number(safeCandles[index].openTime) - Number(safeCandles[index - 1].openTime);
    if (gap !== intervalMs) contiguous = false;
  }
  ...
  return {
    ...
    coverage: {
      ...
      gaps: complete || coverageToBoundary
        ? []
        : [{ start: firstOpen || startTime, end: endTime, reason: "LOWER_TF_COVERAGE_INCOMPLETE" }],
