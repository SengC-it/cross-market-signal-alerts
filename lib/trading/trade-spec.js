...
export function createTradeSpec({
  ...
  return isTradeSpec(tradeSpec) ? tradeSpec : null;
}

export function isTradeSpec(value) {
  const signalCandleOpenTime = toTimestamp(value?.signalCandleOpenTime);
  const signalCandleCloseTime = toTimestamp(value?.signalCandleCloseTime);
  const signalAvailableAt = toTimestamp(value?.signalAvailableAt);
  const entryEligibleAt = toTimestamp(value?.entryEligibleAt);
  const referencePrice = toNumber(value?.referencePrice ?? value?.entry?.referencePrice);
  const stopLoss = toNumber(value?.stopLoss);
  const takeProfit = toNumber(value?.takeProfit);
  const geometryValid = normalizeSide(value?.side) === "SHORT"
    ? Number.isFinite(referencePrice) && Number.isFinite(stopLoss) && Number.isFinite(takeProfit)
  ...
  return Boolean(
    ...
    && Number.isFinite(signalCandleOpenTime)
    && Number.isFinite(signalCandleCloseTime)
    && Number.isFinite(signalAvailableAt)
    && Number.isFinite(entryEligibleAt)
    && signalCandleOpenTime <= signalCandleCloseTime
    && signalCandleCloseTime <= signalAvailableAt
    && signalAvailableAt <= entryEligibleAt
    && Number.isFinite(referencePrice)
    && geometryValid
    && value.entry
    && value.entry.policy === NEXT_BAR_ENTRY_POLICY
    && toTimestamp(value.entry.eligibleAt) === entryEligibleAt
  );
}

export function getTradeSpecForSignal(signal) {
  if (Object.prototype.hasOwnProperty.call(signal || {}, "tradeSpec")) {
    return isTradeSpec(signal.tradeSpec) ? signal.tradeSpec : null;
  }
  const legacyTradeSpec = tradeSpecFromLegacyPayload(signal, {
  ...
  return isTradeSpec(legacyTradeSpec) ? legacyTradeSpec : null;
}

export function getTradeSpecForAlert(alert) {
  const payload = alert?.payload || alert || {};
  if (Object.prototype.hasOwnProperty.call(payload, "tradeSpec")) {
    return isTradeSpec(payload.tradeSpec) ? payload.tradeSpec : null;
  }
  const legacyTradeSpec = tradeSpecFromLegacyPayload(payload, {
