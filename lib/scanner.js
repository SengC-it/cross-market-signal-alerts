...
import { fetchSentAlertsForReview, hasSentSignal, recordRunLog, recordSentSignal, updateSentAlertPayload } from "./storage.js";
...
export async function runSignalScan({ dryRun = false, group = "all", sendEmailNow = true, includeSignals = false } = {}) {
  ...
  if (!dryRun) await reviewRecentSentAlerts({ sentAlerts, warnings });
...
function stripSignalPayloads(result) {
  const { signalPayloads, ...rest } = result;
  return rest;
}
...
async function scanDynamicSpotSymbol({ symbol, candidates, warnings, btc4h, sentAlerts }) {
  ...
  candidates.push({
    ...
    alertTier: "watch",
    alertTierLabel: "观察级",
    opportunityScore: Math.min(score, 82),
    riskScore: 55,
...
export function isDynamicSpotCoolingDown({ sentAlerts, asset, triggerTime }) {
  const latest = sentAlerts
    .filter((alert) =>
      alert.asset === asset &&
      alert.strategy_id === "dynamic_relative_strength_breakout" &&
      alert.interval === "1h"
    )
    .map((alert) => new Date(alert.trigger_time).getTime())
