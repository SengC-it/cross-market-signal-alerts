...
export async function runSignalScan({ dryRun = false, group = "all", sendEmailNow = true, includeSignals = false } = {}) {
  ...
  const dynamicFamilyGate = evaluateDynamicFamilyGate({ sentAlerts });
  const dynamicScanRequested = dynamicSpotAssets.length > 0 || dynamicWeakSpotAssets.length > 0;
  if (dynamicScanRequested && !dynamicFamilyGate.passed) {
    warnings.push({
      label: "dynamic strategy family gate",
      warning: `${dynamicFamilyGate.state}: ${dynamicFamilyGate.reason}`
    });
  }
...
async function scanDynamicSpotSymbol({ group, symbol, candidates, warnings, processedCandles, btc4h, sentAlerts }) {
  ...
  candidates.push(enhanceFuturesSignal(signal, {
    candles,
    strategy,
    interval: "1h",
    funding: null,
...
export function summarizeReviewedFamilyPerformance({ sentAlerts, strategyIds = DYNAMIC_STRATEGY_IDS, defaultTradingCost = CONFIG.futuresTradingCost }) {
  const returns = [];
  for (const alert of sentAlerts || []) {
    if (!strategyIds.has(alert?.strategy_id)) continue;
    const review = alert?.payload?.review;
...
export function evaluateDynamicFamilyGate({ sentAlerts }) {
  const performance = summarizeReviewedFamilyPerformance({ sentAlerts });
