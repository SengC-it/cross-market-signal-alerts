  ...
  if (signals.length && !dryRun && sendEmailNow) {
    try {
      ...
      if (sent?.skipped) {
        ...
        await Promise.all(signals.map((signal) => recordSentSignal(signalRecord(signal))));
    ...
    } else if (!paperSignals.length && emailStatus !== "blocked_by_dedupe_error") {
...
export async function runSignalBatch({ dryRun = false, groups = [] } = {}) {
  ...
  const persistenceBlocked = runs.some((run) => run.persistenceBlocked);
...
function enhanceInverseWatchSignal(signal) {
  return {
    ...signal,
    alertTier: "watch",
    alertTierLabel: "观察级",
...
function enhanceDynamicSignal(signal, options) {
  const enriched = enhanceFuturesSignal(signal, options);
  return withModelMetadata(enriched, {
    modelVersion: DYNAMIC_MODEL_VERSION,
...
function enhanceFuturesSignal(signal, { candles, strategy, interval, funding, openInterest, sentiment }) {
  ...
  const fallbackStopDistance = latest.close * CONFIG.futuresFallbackStopPct;
  const stopDistance = Number.isFinite(latestAtr)
    ? Math.max(latestAtr * CONFIG.futuresStopAtrMultiplier, latest.close * CONFIG.futuresMinStopPct)
  ...
  const executionPlan = {
    ...
    maxHoldingHours: CONFIG.futuresMaxHoldingHours,
...
function summarizeLiveAlertPerformance({ sentAlerts, candles, asset, strategy, interval, tradingCost, modelVersion = null }) {
...
export function summarizeReviewedFamilyPerformance({
  ...
  for (const alert of relevantAlerts) {
    ...
    const modeledCost = Number(alert?.payload?.executionPlan?.modeledRoundTripCostPct);
...
export function isDynamicSpotCoolingDown({ sentAlerts, asset, triggerTime, modelVersion = null }) {
...
function isDynamicStrategyCoolingDown({ sentAlerts, asset, strategyId, interval, triggerTime, modelVersion = null }) {
  const latest = sentAlerts
    .filter((alert) =>
      alert.asset === asset &&
