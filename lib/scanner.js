...
export async function runSignalScan({ dryRun = false, group = "all", sendEmailNow = true, includeSignals = false } = {}) {
  ...
  const sentAlerts = await safe(
    () => fetchRecentSentAlerts(CONFIG.livePerformanceFetchLimit),
    errors,
    "sent alerts review lookup"
  ) ?? [];
  // Review persistence runs in the dedicated recovery queue. The scanner still
  // uses recent history for gates and email performance context.
  const emailPerformanceHistory = sentAlerts;
