...
export async function runSignalScan({ dryRun = false, group = "all" } = {}) {
  ...
  await Promise.all(selected.cryptoAssets.map((symbol) => scanCryptoSymbol({ symbol, candidates, warnings, btc4h })));
  await Promise.all(selected.futuresAssets.map((symbol) => scanFuturesSymbol({ symbol, candidates, warnings, btc4h })));
  await Promise.all(selected.arbitrageAssets.map((symbol) => scanFuturesArbitrageSymbol({ symbol, candidates, warnings })));

  const spyBenchmark = selected.tradfiAssets.length
    ? await safeMarket(() => getYahooCandles("SPY", "5y", "1d"), warnings, "SPY benchmark")
    : null;
  await Promise.all(selected.tradfiAssets.map((symbol) => scanTradfiSymbol({ symbol, candidates, warnings, spyBenchmark })));
...
async function scanCryptoSymbol({ symbol, candidates, warnings, btc4h }) {
...
async function scanFuturesSymbol({ symbol, candidates, warnings, btc4h }) {
  ...
  await Promise.all([...CONFIG.intervals, ...CONFIG.shortTermIntervals].map(async (interval) => {
    const candles = await safeMarket(() => getFuturesCandles(symbol, interval, 1000), warnings, `${symbol} futures ${interval}`);
    if (!candles || candles.length < 260) return;
...
async function scanFuturesArbitrageSymbol({ symbol, candidates, warnings }) {
  ...
  if (annualizedMagnitude < CONFIG.arbitrageAnnualizedThreshold) {
    warnings.push({
      label: `${symbol} arbitrage threshold`,
      warning: `资金费年化 ${formatPct(annualizedFunding)} 低于提醒阈值 ${formatPct(CONFIG.arbitrageAnnualizedThreshold)}`
    });
    return;
  }
...
async function scanTradfiSymbol({ symbol, candidates, warnings, spyBenchmark }) {
