...
function enhanceInverseWatchSignal(signal, { strategy, interval } = {}) {
  const referencePrice = Number(signal.close);
  ...
  const tradeSpec = createTradeSpec({
    ...
    referencePrice,
    stopLoss: isShort ? referencePrice + stopDistance : referencePrice - stopDistance,
    takeProfit: isShort
      ? referencePrice - stopDistance * (CONFIG.futuresRewardRiskRatio || 1.5)
      : referencePrice + stopDistance * (CONFIG.futuresRewardRiskRatio || 1.5),
    rewardRiskRatio: CONFIG.futuresRewardRiskRatio || 1.5,
    maxHoldingHours: strategy?.holdHours,
    modelVersion: "inverse_watch_trade_spec_v1"
  });
  const enrichedSignal = attachTradeSpec(signal, tradeSpec);
  return {
    ...enrichedSignal,
    alertTier: "watch",
    alertTierLabel: "观察级",
...
function buildSignal({ asset, market, interval, strategy, metrics, filters, recommendationScore, currentSignal, triggerSuffix = "", livePerformance = null, gateNotes = [], scoringBreakdown = null, alertTier = "trade", alertTierLabel = "交易级", opportunityScore = null, riskScore = null, tierReason = "" }) {
  ...
  return attachTradeSpec(signal, tradeSpec);
}
...
function summarizeLiveAlertPerformance({ sentAlerts, candles, asset, strategy, interval, tradingCost, modelVersion = null }) {
  ...
  for (const alert of relevant) {
    ...
    const entryTime = Number(tradeSpec?.signalCandleOpenTime ?? new Date(alert.trigger_time).getTime());
