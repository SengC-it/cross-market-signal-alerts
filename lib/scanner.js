...
export async function runSignalScan({ dryRun = false, group = "all", sendEmailNow = true, includeSignals = false } = {}) {
  ...
  let policyCandidates = [
    ...routedSignals.emailCandidates,
    ...routedSignals.observationSignals,
  ...
  const emailCandidates = routedSignals.emailCandidates
    .map((signal) => guardedByKey.get(signal.signalKey))
    .filter(Boolean);
  const observationCandidates = routedSignals.observationSignals
    .map((signal) => guardedByKey.get(signal.signalKey))
    .filter(Boolean);
  ...
  const emailSelection = await filterUnrecordedSignals({
    signals: emailCandidates,
    errors,
    label: "sent alert"
  });
  signals = emailSelection.signals;
  ...
  if (persistenceBlocked) emailStatus = "blocked_by_dedupe_error";
  if (observationSignalKeys.length || shadowOnlySignalKeys.length) {
    warnings.push({
      label: "signal-only non-email queues",
      warning: `${observationSignalKeys.length} observation signal(s) and ${shadowOnlySignalKeys.length} shadow-only signal(s) recorded without normal trading email`
    });
  }
  ...
  const result = {
    ...
    observationSignals: observationCandidates.length,
