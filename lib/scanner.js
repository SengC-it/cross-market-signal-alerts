...
import { loadSignalEmailPerformanceSnapshot } from "./email-performance-context.js";
...
import {
  DYNAMIC_STRATEGY_IDS as DYNAMIC_PRODUCTION_STRATEGY_IDS,
  DYNAMIC_PRODUCTION_HOLD_HOURS,
  benchmarkMomentum24h as benchmarkMomentum24hFromProduction,
...
} from "./strategies/strong-extension.js";
import { SIGNAL_DENSITY_CONFIG } from "./signal-density-config.js";
import { dynamicPoolRankBand, rankDynamicPoolTickerDetails } from "./signal-density-pool.js";
import {
  SIGNAL_ONLY_RELEASE,
  isDynamicStrengthObservation,
  markSignalAsWebOverflow,
  routeSignalsByProductionPolicy
} from "./production-signal-policy.js";

const DYNAMIC_STRATEGY_IDS = new Set(DYNAMIC_PRODUCTION_STRATEGY_IDS);
...
export async function runSignalScan({ dryRun = false, group = "all", sendEmailNow = true, includeSignals = false } = {}) {
  ...
  const dynamicSpotPool = await loadDynamicSpotAssets({ group, selected, warnings });
  const dynamicSpotAssets = dynamicSpotPool.assets;
  const dynamicExtensionAssets = dynamicSpotPool.extensionAssets;
  const dynamicStrongAssets = [...dynamicSpotAssets, ...dynamicExtensionAssets];
  const dynamicWeakSpotAssets = await loadDynamicWeakSpotAssets({ group, selected, dynamicSpotAssets: dynamicStrongAssets, warnings });
  ...
  const emailPerformanceHistory = !dryRun && shouldReviewRecentAlerts(group)
    ? await reviewRecentSentAlerts({ sentAlerts, warnings })
    : sentAlerts;
  ...
  const dynamicScanRequested = dynamicStrongAssets.length > 0 || dynamicWeakSpotAssets.length > 0;
  if (dynamicScanRequested) {
    for (const [family, gate] of Object.entries(dynamicFamilyGates)) {
      if (!gate.passed) {
        warnings.push({
          label: `dynamic ${family} gate`,
          warning: `${gate.state}: ${gate.reason}; live-family evidence is diagnostic only, and the SIGNAL-ONLY tier policy remains in force`
        });
      }
    }
  }

  const needsCryptoBenchmark = selected.cryptoAssets.length || selected.futuresAssets.length || dynamicStrongAssets.length || dynamicWeakSpotAssets.length;
  ...
  await mapLimit(dynamicSpotAssets, CONFIG.marketDataConcurrency, (symbol) => {
    const poolRank = dynamicSpotPool.rankByAsset.get(symbol);
    return scanDynamicSpotSymbol({
      group,
      symbol,
      candidates,
      warnings,
      ...
      btc4h,
      sentAlerts,
      dynamicPoolRank: poolRank?.dynamicPoolRank,
      dynamicPoolRankBand: poolRank?.dynamicPoolRankBand,
      signalVariant: STRONG_CORE_VARIANT
    });
  });
  await mapLimit(dynamicExtensionAssets, CONFIG.marketDataConcurrency, (symbol) => {
    const poolRank = dynamicSpotPool.extensionRankByAsset.get(symbol);
    return scanDynamicSpotSymbol({
      group,
      symbol,
      candidates,
  ...
  const routedSignals = routeSignalsByProductionPolicy({
    candidates,
    limit: SIGNAL_DENSITY_CONFIG.maxSignalsPerEmail,
    strengthObservationEmailEnabled: SIGNAL_ONLY_RELEASE.dynamicStrengthObservationEmailEnabled
  });
  let signals = [];
  let observationSignals = [];
  let shadowOnlySignals = [];
  let persistenceBlocked = false;

  if (routedSignals.researchOnlySignals.length) {
    warnings.push({
      label: "research-only production block",
      warning: `${routedSignals.researchOnlySignals.length} frozen research candidate(s) were excluded from production alerts`
    });
  }

  let policyCandidates = [
    ...routedSignals.emailEligibleSignals,
    ...routedSignals.webSignals,
    ...routedSignals.shadowOnlySignals
  ];
  if (!dryRun) {
    policyCandidates = await guardSignalsByCurrentPrice({ signals: policyCandidates, warnings });
  }

  const guardedByKey = new Map(policyCandidates.map((signal) => [signal.signalKey, signal]));
  const emailEligibleCandidates = routedSignals.emailEligibleSignals
    .map((signal) => guardedByKey.get(signal.signalKey))
    .filter(Boolean);
  const observationCandidates = routedSignals.webSignals
    .map((signal) => guardedByKey.get(signal.signalKey))
    .filter(Boolean);
  const shadowCandidates = routedSignals.shadowOnlySignals
    .map((signal) => guardedByKey.get(signal.signalKey))
    .filter(Boolean);

  const emailSelection = await filterUnrecordedSignals({
    signals: emailEligibleCandidates,
    errors,
    label: "sent alert"
  });
  const emailCapacity = sendEmailNow
    ? splitEmailCapacity({ signals: emailSelection.signals, limit: SIGNAL_DENSITY_CONFIG.maxSignalsPerEmail })
    : { emailSignals: emailSelection.signals, overflowSignals: [] };
  signals = emailCapacity.emailSignals;
  const emailOverflowSignals = emailCapacity.overflowSignals;
  persistenceBlocked ||= emailSelection.persistenceBlocked;
  const observationSelection = await filterUnrecordedSignals({
    signals: observationCandidates,
    errors,
    label: "observation signal"
  });
  observationSignals = observationSelection.signals;
  persistenceBlocked ||= observationSelection.persistenceBlocked;
  const shadowSelection = await filterUnrecordedSignals({
    signals: shadowCandidates,
    errors,
    label: "shadow-only signal"
  });
  shadowOnlySignals = shadowSelection.signals;
  persistenceBlocked ||= shadowSelection.persistenceBlocked;
  if (persistenceBlocked) emailStatus = "blocked_by_dedupe_error";

  const observationPersistence = !dryRun
    ? await persistNonEmailSignals({
      signals: observationSignals,
      errors,
      label: "observation signal"
    })
    : { keys: [], persistenceBlocked: false };
  const observationSignalKeys = observationPersistence.keys;
  persistenceBlocked ||= observationPersistence.persistenceBlocked;
  const shadowPersistence = !dryRun
    ? await persistNonEmailSignals({
      signals: shadowOnlySignals,
      errors,
      label: "shadow-only signal"
    })
    : { keys: [], persistenceBlocked: false };
  const shadowOnlySignalKeys = shadowPersistence.keys;
  persistenceBlocked ||= shadowPersistence.persistenceBlocked;
  const overflowPersistence = !dryRun
    ? await persistNonEmailSignals({
      signals: emailOverflowSignals.map((signal) => markSignalAsWebOverflow(signal)),
      errors,
      label: "email overflow signal"
    })
    : { keys: [], persistenceBlocked: false };
  const overflowSignalKeys = overflowPersistence.keys;
  persistenceBlocked ||= overflowPersistence.persistenceBlocked;
  if (persistenceBlocked) emailStatus = "blocked_by_dedupe_error";
  if (observationSignalKeys.length || shadowOnlySignalKeys.length || overflowSignalKeys.length) {
    warnings.push({
      label: "signal-only non-email queues",
      warning: `${observationSignalKeys.length} observation, ${shadowOnlySignalKeys.length} shadow-only, and ${overflowSignalKeys.length} email-overflow signal(s) recorded without normal trading email`
    });
  }

  const queueStats = buildSignalQueueStats({
    eligibleHighQualitySignals: routedSignals.eligibleHighQualitySignals,
    eligibleStrongObservations: routedSignals.eligibleStrongObservations,
    emailedSignals: [],
    overflowSignals: emailOverflowSignals
  });
  ...
  if (signals.length && !dryRun && sendEmailNow) {
    try {
      const performanceSnapshot = await loadSignalEmailPerformanceSnapshot({
        signals,
        historySignals: emailPerformanceHistory
      });
      const email = {
        ...renderSignalEmail(signals, { performanceSnapshot }),
      ...
      if (sent?.skipped) {
        ...
        emailResult = summarizeEmailResult(sent);
        queueStats.emailedHighQualitySignals = signals.filter((signal) => signal.signalTier === "TRADE_WATCH").length;
        queueStats.emailedStrongObservations = signals.filter(isDynamicStrengthObservation).length;
        emailed = true;
        emailStatus = "sent";
      }
    } catch (error) {
    ...
    if (dryRun) {
      emailStatus = shadowCandidates.length || observationCandidates.length
        ? "dry_run_non_email_signals"
        : "dry_run_no_signals";
    } else if (
      !observationSignalKeys.length
      && !shadowOnlySignalKeys.length
      && !["blocked_by_dedupe_error", "deduplicated_in_flight"].includes(emailStatus)
    ) {
      emailStatus = "no_signals";
    } else if (!emailStatus.startsWith("blocked") && !emailStatus.startsWith("deduplicated")) {
      emailStatus = observationSignalKeys.length && shadowOnlySignalKeys.length
        ? "observation_and_shadow_recorded"
        : observationSignalKeys.length
          ? "observation_recorded"
          : "shadow_only_recorded";
    }
  }
  ...
  await recordRunLog({
    ...
    email_result: withSignalQueueStats(emailResult, queueStats),
  ...
  const result = {
    ...
    highQualitySignals: signals.length,
    observationSignals: observationCandidates.length + routedSignals.strongObservationSignals.length,
    observationSignalsRecorded: observationSignalKeys.length,
    observationSignalKeys,
    overflowSignals: emailOverflowSignals.length,
    overflowSignalKeys,
    shadowOnlySignals: shadowCandidates.length,
    shadowOnlySignalsRecorded: shadowOnlySignalKeys.length,
    shadowOnlySignalKeys,
    researchOnlySignals: routedSignals.researchOnlySignals.length,
    paperSignals: 0,
    paperSignalsRecorded: 0,
    paperSignalKeys: [],
    dynamicFamilyGate,
    dynamicFamilyGates,
    persistenceBlocked,
  ...
  if (includeSignals) result.emailPerformanceHistory = emailPerformanceHistory;
...
export async function runSignalBatch({ dryRun = false, groups = [] } = {}) {
  ...
  const emailPerformanceHistoryByKey = new Map();
  await mapLimit(normalizedGroups, CONFIG.batchGroupConcurrency, async (group) => {
    ...
    for (const alert of result.emailPerformanceHistory || []) {
      const key = alert?.signal_key || alert?.signalKey;
      if (key && !emailPerformanceHistoryByKey.has(key)) emailPerformanceHistoryByKey.set(key, alert);
    }
  ...
  const observationSignals = runs.reduce((sum, run) => sum + Number(run.observationSignals || 0), 0);
  const shadowOnlySignals = runs.reduce((sum, run) => sum + Number(run.shadowOnlySignals || 0), 0);
  const researchOnlySignals = runs.reduce((sum, run) => sum + Number(run.researchOnlySignals || 0), 0);
  let persistenceBlocked = runs.some((run) => run.persistenceBlocked);
  const eligibleHighQualitySignals = runs.reduce(
    (sum, run) => sum + Number(run.queueStats?.eligibleHighQualitySignals || 0),
    0
  );
  const eligibleStrongObservations = runs.reduce(
    (sum, run) => sum + Number(run.queueStats?.eligibleStrongObservations || 0),
    0
  );
  const allEmailSignals = [...signalsByKey.values()]
    .sort((a, b) => Number(b.recommendationScore || 0) - Number(a.recommendationScore || 0));
  const guardedEmailSignals = dryRun
    ? allEmailSignals
    : await guardSignalsByCurrentPrice({ signals: allEmailSignals, warnings });
  const emailCapacity = splitEmailCapacity({
    signals: guardedEmailSignals,
    limit: SIGNAL_DENSITY_CONFIG.maxSignalsPerEmail
  });
  let signals = emailCapacity.emailSignals;
  const emailOverflowSignals = emailCapacity.overflowSignals;
  const queueStats = buildSignalQueueStats({
    eligibleHighQualitySignals,
    eligibleStrongObservations,
    emailedSignals: [],
    overflowSignals: emailOverflowSignals
  });
  const overflowPersistence = !dryRun
    ? await persistNonEmailSignals({
      signals: emailOverflowSignals.map((signal) => markSignalAsWebOverflow(signal)),
      errors,
      label: "batch email overflow signal"
    })
    : { keys: [], persistenceBlocked: false };
  const overflowSignalKeys = overflowPersistence.keys;
  persistenceBlocked ||= overflowPersistence.persistenceBlocked;
  ...
  if (signals.length && !dryRun) {
    try {
      const performanceSnapshot = await loadSignalEmailPerformanceSnapshot({
        signals,
        historySignals: [...emailPerformanceHistoryByKey.values()]
      });
      const email = {
        ...renderSignalEmail(signals, { performanceSnapshot }),
        idempotencyKey: buildEmailIdempotencyKey(signals)
    ...
    emailStatus = dryRun
      ? observationSignals || shadowOnlySignals ? "dry_run_non_email_signals" : "dry_run_no_signals"
      : observationSignals || shadowOnlySignals
        ? persistenceBlocked ? "non_email_persistence_partial" : "non_email_signals_recorded"
        : ["blocked_by_dedupe_error", "deduplicated_in_flight"].includes(emailStatus)
          ? emailStatus
          : "no_signals";
  }
  ...
  return {
    ...
    highQualitySignals: signals.length,
    observationSignals,
    observationSignalsRecorded: runs.reduce(
      (sum, run) => sum + Number(run.observationSignalsRecorded || 0),
      0
    ),
    shadowOnlySignals,
    shadowOnlySignalsRecorded: runs.reduce(
      (sum, run) => sum + Number(run.shadowOnlySignalsRecorded || 0),
      0
    ),
    researchOnlySignals,
    paperSignals: 0,
...
function stripSignalPayloads(result) {
  const { signalPayloads, emailPerformanceHistory, processedCandles, ...rest } = result;
  return rest;
}

export function splitEmailCapacity({ signals = [], limit = SIGNAL_DENSITY_CONFIG.maxSignalsPerEmail } = {}) {
  ...
  const sorted = [...signals].sort((left, right) => {
    ...
    return scoreDelta || String(left.signalKey || "").localeCompare(String(right.signalKey || ""));
  });
  return {
    emailSignals: sorted.slice(0, normalizedLimit),
    overflowSignals: sorted.slice(normalizedLimit)
  };
}

export function buildSignalQueueStats({
  eligibleHighQualitySignals = 0,
  eligibleStrongObservations = 0,
  emailedSignals = [],
  overflowSignals = []
} = {}) {
  return {
    eligibleHighQualitySignals: Number(eligibleHighQualitySignals) || 0,
    emailedHighQualitySignals: emailedSignals.filter((signal) => signal.signalTier === "TRADE_WATCH").length,
    highQualityOverflowSignals: overflowSignals.filter((signal) => signal.signalTier === "TRADE_WATCH").length,
    eligibleStrongObservations: Number(eligibleStrongObservations) || 0,
    emailedStrongObservations: emailedSignals.filter(isDynamicStrengthObservation).length,
    observationOverflowSignals: overflowSignals.filter(isDynamicStrengthObservation).length
  };
}

function withSignalQueueStats(emailResult, queueStats) {
  return {
    ...(emailResult || {}),
    signalQueue: queueStats
  };
}

async function filterUnrecordedSignals({ signals = [], errors, label }) {
  const unrecorded = [];
  let persistenceBlocked = false;
  for (const signal of signals) {
    const alreadyRecorded = await safe(
      () => hasSentSignal(signal.signalKey),
      errors,
      `${label} lookup ${signal.signalKey}`
    );
    if (alreadyRecorded === null) {
      persistenceBlocked = true;
      continue;
    }
    if (!alreadyRecorded) unrecorded.push(signal);
  }
  return { signals: unrecorded, persistenceBlocked };
}

async function persistNonEmailSignals({ signals = [], errors, label }) {
  const keys = [];
  let persistenceBlocked = false;
  for (const signal of signals) {
    try {
      await recordSentSignal(signalRecord(signal));
      keys.push(signal.signalKey);
    } catch (error) {
      persistenceBlocked = true;
      errors.push({
        label: `${label} record ${signal.signalKey}`,
        error: error instanceof Error ? error.message : String(error)
      });
    }
  }
  return { keys, persistenceBlocked };
}
...
async function guardSignalsByCurrentPrice({ signals, warnings }) {
  ...
  return guarded.map((signal) => {
    ...
    return {
      ...
      priceDriftPct: Number.isFinite(current) && Number.isFinite(reference) && reference > 0
        ? current / reference - 1
        : null
    };
  });
...
export function evaluateDynamicSpotOpportunity({ momentum24h, relativeStrength, volumeMultiple, breakout, hasOrderBook = false }) {
  return evaluateDynamicProductionOpportunity({
...
export function evaluateDynamicProductionSignalForStrategy(options) {
  return evaluateDynamicProductionSignal(options);
}

async function scanDynamicSpotSymbol({
  group,
  symbol,
  candidates,
  warnings,
  ...
  btc4h,
  sentAlerts,
  dynamicPoolRank = null,
  dynamicPoolRankBand: poolRankBand = null,
  signalVariant = STRONG_CORE_VARIANT
}) {
  ...
  const productionEvaluation = signalVariant === STRONG_EXTENSION_VARIANT
    ? evaluateStrongExtensionProductionSignal({
      candles,
      signalIndex: latestIndex,
      benchmarkChange24h,
  ...
  const { features, opportunity } = productionEvaluation;
  ...
  const score = productionEvaluation.score;
  if (!productionEvaluation.scoreGatePassed) return;

  const signalKey = signalVariant === STRONG_EXTENSION_VARIANT
    ? `${symbol}:DYNAMIC_SPOT:1h:dynamic_relative_strength_breakout:${latest.openTime}:${signalVariant}:${DYNAMIC_MODEL_VERSION}`
    : `${symbol}:DYNAMIC_SPOT:1h:dynamic_relative_strength_breakout:${latest.openTime}:${DYNAMIC_MODEL_VERSION}`;
  const signal = {
    signalKey,
    asset: symbol,
    market: signalVariant === STRONG_EXTENSION_VARIANT
      ? "USDT 永续合约（STRONG EXTENSION 10–15%）"
      : "USDT 永续合约（动态强势池）",
    interval: "1h",
    strategyId: "dynamic_relative_strength_breakout",
    strategyName: signalVariant === STRONG_EXTENSION_VARIANT
      ? "STRONG EXTENSION 10–15% / 动态强势扩展"
      : "动态强势币放量突破",
    signalVariant,
    direction: "做多观察",
    signalCandleOpenTime: latest.openTime,
    ...
    livePerformance,
    gateNotes: [signalVariant === STRONG_EXTENSION_VARIANT
      ? "STRONG EXTENSION 10–15%：仅观察级，未验证盈利能力，REFERENCE ONLY"
      : "动态强势池：历史样本可能不足，只作为短线观察，不作为高置信交易信号"],
    scoringBreakdown: {
      outOfSample: 6,
      environment: 16,
    ...
    riskScore: 55,
    dynamicPoolRank: Number.isFinite(Number(dynamicPoolRank)) ? Number(dynamicPoolRank) : null,
    dynamicPoolRankBand: poolRankBand || dynamicPoolRankBand(dynamicPoolRank),
    tierReason: signalVariant === STRONG_EXTENSION_VARIANT
      ? "STRONG EXTENSION 10–15% 只作观察，不作为直接交易指令。"
      : "动态强势池只提示短线观察，不作为直接交易指令。",
    triggerReason: signalVariant === STRONG_EXTENSION_VARIANT
      ? "该资产进入 STRONG EXTENSION 10–15% 池，并在 1h 级别出现放量相对强势突破。"
      : "该资产进入动态强势池，并在 1h 级别出现放量相对强势突破。",
...
async function scanDynamicWeakSpotSymbol({ group, symbol, candidates, warnings, processedCandles, btc4h, sentAlerts }) {
  ...
  const latest = candles[latestIndex];
  const benchmarkChange24h = benchmarkMomentum24h(btc4h);
  const productionEvaluation = evaluateDynamicProductionSignal({
...
export function enhanceInverseWatchSignal(signal, { strategy, interval } = {}) {
...
function addCandidate({ candidates, candles, strategy, interval, tradingCost, filters, asset, market, triggerSuffix = "", sentAlerts = [], signalEnhancer = null, tradePlanType = null }) {
  ...
  const metrics = backtestStrategy(candles, strategy, interval, tradingCost, {
    marketType,
    tradePlanType: tradePlanType || marketType,
    asset
  });
...
async function reviewRecentSentAlerts({ sentAlerts, warnings }) {
  ...
  await mapLimit(reviewable, 2, async (alert) => {
    try {
      ...
      updatedByKey.set(alert.signal_key, { ...alert, payload: { ...payload, review } });
    } catch (error) {
      warnings.push({
  ...
  return sentAlerts.map((alert) => updatedByKey.get(alert.signal_key) || alert);
}
...
export function summarizeLiveAlertPerformance({ sentAlerts, candles, asset, strategy, interval, tradingCost, modelVersion = null }) {
  ...
  const persistedTradeSpecAlerts = relevant.filter((alert) =>
    Object.prototype.hasOwnProperty.call(alert?.payload || {}, "tradeSpec")
    && isTradeSpec(alert.payload.tradeSpec)
    && alert.payload.tradeSpec.source !== "legacy_adapter"
  );
  const v4AlertKeys = new Set(persistedTradeSpecAlerts.map((alert) => alert.signal_key || alert.signalKey || alert.trigger_time));
  const reviewedReturns = [];
  const legacyReturns = [];
  for (const alert of relevant) {
    const alertKey = alert.signal_key || alert.signalKey || alert.trigger_time;
    if (v4AlertKeys.has(alertKey)) {
      const review = alert.payload?.review;
      if (review?.status !== "reviewed" || review.state === "ambiguous" || review.reason === "pending_partial_candle") continue;
      const reviewedReturn = Number(review.netReturnPct ?? review.returnPct);
      if (Number.isFinite(reviewedReturn)) reviewedReturns.push(reviewedReturn);
      continue;
    }

    const tradeSpec = getTradeSpecForAlert(alert);
    if (tradeSpec?.source !== "legacy_adapter") continue;
    const entryTime = Number(tradeSpec.entryEligibleAt);
    const maxHoldingTime = Number(tradeSpec.maxHoldingTime);
    if (!Number.isFinite(entryTime) || !Number.isFinite(maxHoldingTime) || maxHoldingTime <= entryTime) continue;
    ...
    legacyReturns.push((exit.close / entry - 1) * directionMultiplier - (Number(tradingCost) || 0));
  }
  const useV4Reviews = persistedTradeSpecAlerts.length > 0;
  const returns = useV4Reviews ? reviewedReturns : legacyReturns;
  if (!returns.length) {
    return {
      trades: 0,
      winRate: null,
      totalReturn: 0,
      averageReturn: null,
      dataQuality: useV4Reviews ? "REVIEWED_TRADESPEC" : "LEGACY_DEGRADED",
      source: useV4Reviews ? "persisted_review" : "legacy_fixed_hold_fallback"
    };
  }
  let equity = 1;
  let wins = 0;
...
async function loadDynamicSpotAssets({ group, selected, warnings }) {
  if (!CONFIG.dynamicSpotPoolEnabled || !["all", "dynamic-spot"].includes(group)) {
    return {
      assets: [],
      rankByAsset: new Map(),
      extensionAssets: [],
      extensionRankByAsset: new Map()
    };
  }
  ...
  const existing = resolveDynamicPoolExistingAssets({ group, selected });
  const ranked = rankDynamicPoolTickerDetails({
    tickers,
    direction: "strong",
    existing,
    futuresSymbols: futuresSymbolSet,
    maxAssets: SIGNAL_DENSITY_CONFIG.dynamicStrongPoolMaxAssets
  });
  const extensionAssets = rankStrongExtensionPoolTickers({
    tickers,
    existing,
    futuresSymbols: futuresSymbolSet,
    maxAssets: SIGNAL_DENSITY_CONFIG.dynamicStrongPoolMaxAssets
  ...
  return {
    assets: ranked.map((row) => row.symbol),
    rankByAsset: new Map(ranked.map((row) => [row.symbol, row])),
    extensionAssets,
    extensionRankByAsset: new Map(extensionAssets.map((symbol, index) => [symbol, {
...
async function loadDynamicWeakSpotAssets({ group, selected, dynamicSpotAssets, warnings }) {
  ...
  const futuresSymbolSet = new Set(futuresSymbols);
  const existing = resolveDynamicPoolExistingAssets({ group, selected, dynamicSpotAssets });
  return rankDynamicPoolTickers({
    tickers,
    direction: "weak",
    existing,
    futuresSymbols: futuresSymbolSet,
...
export function isDynamicSpotCandidate(ticker, existing = new Set(), futuresSymbols = null) {
  return isDynamicStrongPoolCandidate(ticker, existing, futuresSymbols);
}

export function isDynamicWeakSpotCandidate(ticker, existing = new Set(), futuresSymbols = null) {
  return isDynamicWeakPoolCandidate(ticker, existing, futuresSymbols);
}

function benchmarkMomentum24h(candles) {
  return benchmarkMomentum24hFromProduction(candles);
}
