...
function scoreFuturesSentiment({ checks, sentiment, wantsLong, wantsShort }) {
  let score = 0;
  ...
  if (Number.isFinite(topRatio)) {
    if ((wantsLong && topRatio >= 1.05) || (wantsShort && topRatio <= 0.95)) {
      checks.push(["大户倾向", "通过", `顶级交易员持仓多空比 ${topRatio.toFixed(2)}，与方向较一致`]);
      score += 3;
    } else if ((wantsLong && topRatio > 2.5) || (wantsShort && topRatio < 0.4)) {
