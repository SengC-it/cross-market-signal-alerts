...
export function backtestStrategy(candles, strategy, interval, tradingCost, options = {}) {
  ...
  if (!result.tradeResults.length) return null;
...
export function compareStrategyInversion({ candles, strategy, interval, tradingCost, minTrades = CONFIG.minTrades, marketType = "futures", executionModel = null }) {
  const inverseStrategy = invertStrategyDirection(strategy);
  const original = backtestStrategy(candles, strategy, interval, tradingCost, { marketType, executionModel });
  const inverse = backtestStrategy(candles, inverseStrategy, interval, tradingCost, { marketType, executionModel });
