...
import { createLegacyExecutionModel } from "./backtest/execution-model.js";
...
export const SHORT_TERM_STRATEGIES = [
  {
    ...
    evaluate(candles, index, context = {}) {
      ...
      const volumeMultiple = volume20 ? volumes[index] / volume20 : null;
      return {
        passed:
          momentum > 0.04 &&
          momentum <= CONFIG.relativeStrengthMaxMomentum24h &&
          volumeMultiple != null &&
          volumeMultiple > 1 &&
          volumeMultiple <= CONFIG.relativeStrengthMaxVolumeMultiple,
        details: { close: closes[index], momentum24h: momentum, lookbackBars, volumeMultiple }
      };
    }
  },
  {
...
export const PRODUCTION_SHORT_TERM_STRATEGIES = SHORT_TERM_STRATEGIES.filter((strategy) =>
  strategy.id !== "short_term_momentum_24h"
);

export const CRYPTO_STRATEGIES = [...STRATEGIES, ...BREAKDOWN_STRATEGIES, ...PRODUCTION_SHORT_TERM_STRATEGIES];

export const FUTURES_SCALP_STRATEGIES = [
  {
    id: "futures_scalp_volume_breakout",
    name: "U本位短线放量突破",
    direction: "LONG",
    holdHours: 2,
    evaluate(candles, index, context = {}) {
      if (!["15m", "30m"].includes(context.interval)) return { passed: false, details: {} };
      const closes = candles.map((c) => c.close);
      const highs = candles.map((c) => c.high);
      const volumes = candles.map((c) => c.volume);
      ...
      const volume20 = sma(volumes, 20, index);
      const value = rsi(closes, 14, index);
      const latestAtr = atr(candles, 14, index);
      const atrRatio = latestAtr && closes[index] ? latestAtr / closes[index] : null;
      return {
        passed: high20 != null && volume20 != null && value != null && closes[index] > high20 && volumes[index] > volume20 * 1.35 && value < 76 && atrRatio != null && atrRatio < 0.035,
        details: { close: closes[index], breakoutLevel: high20, volumeMultiple: volumes[index] / volume20, rsi14: value, atrRatio }
      };
    }
  },
  {
    id: "futures_scalp_breakdown",
    name: "U本位短线放量跌破",
    direction: "SHORT",
    holdHours: 2,
    evaluate(candles, index, context = {}) {
      ...
      const atrRatio = latestAtr && closes[index] ? latestAtr / closes[index] : null;
      return {
        passed: low20 != null && volume20 != null && value != null && closes[index] < low20 && volumes[index] > volume20 * 1.35 && value > 24 && atrRatio != null && atrRatio < 0.04,
        details: { close: closes[index], breakdownLevel: low20, volumeMultiple: volumes[index] / volume20, rsi14: value, atrRatio }
      };
    }
  },
  {
    id: "futures_scalp_trend_pullback",
    name: "U本位短线强势回踩",
    direction: "LONG",
    holdHours: 3,
    evaluate(candles, index, context = {}) {
      ...
      const prev = rsi(closes, 14, index - 1);
      return {
        passed: ma20 != null && ma50 != null && value != null && prev != null && closes[index] > ma50 && ma20 > ma50 && prev < 45 && value >= 45,
        details: { close: closes[index], sma20: ma20, sma50: ma50, rsi14: value }
      };
    }
  }
];
...
export const FUTURES_STRATEGIES = [
  ...
  ...PRODUCTION_SHORT_TERM_STRATEGIES,
  ...FUTURES_SCALP_STRATEGIES,
  {
...
export function backtestStrategy(candles, strategy, interval, tradingCost, options = {}) {
  const marketType = options.marketType || strategy.marketType || "futures";
  const tradePlanType = options.tradePlanType || marketType;
  const executionModel = options.executionModel || createLegacyExecutionModel(tradingCost, marketType);
  const result = runBacktest({
    ...
    startIndex: options.startIndex ?? 220,
    executionModel,
    fundingEvents: options.fundingEvents,
    fundingDataComplete: options.fundingDataComplete,
    fundingCoverage: options.fundingCoverage,
    lowerTimeframeCandles: options.lowerTimeframeCandles,
    lowerTimeframe: options.lowerTimeframe,
    exchangeFilters: options.exchangeFilters,
    requestedQty: options.requestedQty,
    orderType: options.orderType
  });
  if (!result.tradeResults.length && !result.missedEntries.length) return null;

  const returns = result.tradeResults.map((trade) => trade.netReturnPct);
  const years = (candles.at(-1).openTime - candles[0].openTime) / (365.25 * 24 * 3600 * 1000);
  const equity = 1 + result.metrics.totalNetReturn;
  const legacyTrades = result.tradeResults.map((trade) => ({
    value: trade.netReturnPct,
    signalTime: trade.signalAvailableAt,
    signalCandleOpenTime: trade.signalCandleOpenTime,
    signalAvailableAt: trade.signalAvailableAt,
    entryEligibleAt: trade.entryEligibleAt,
  ...
  return {
    ...
    averageReturn: result.metrics.averageNetReturn,
    expectancy: result.metrics.averageNetReturn,
    payoffRatio: result.metrics.payoffRatio,
    totalReturn: result.metrics.totalNetReturn,
    cagr: years > 0 ? Math.pow(equity, 1 / years) - 1 : result.metrics.totalNetReturn,
    ...
    outOfSample,
    recent: summarizeRecentTrades({ candles, trades: legacyTrades }),
    tradeResults: result.tradeResults,
    metrics: result.metrics,
    dataQuality: result.dataQuality,
    marketType: result.marketType,
    tradePlanType: result.tradePlanType,
    missedEntries: result.missedEntries,
    entryStats: result.entryStats
  };
}

export function invertStrategyDirection(strategy) {
  return {
    ...strategy,
    id: `${strategy.id}__inverse`,
    name: `${strategy.name} inverse`,
    direction: strategy.direction === "SHORT" ? "LONG" : "SHORT",
    inverseOf: strategy.id,
    evaluate: strategy.evaluate.bind(strategy)
  };
}

export function compareStrategyInversion({ candles, strategy, interval, tradingCost, minTrades = CONFIG.minTrades, marketType = "futures", tradePlanType = marketType, executionModel = null }) {
  const inverseStrategy = invertStrategyDirection(strategy);
  const original = backtestStrategy(candles, strategy, interval, tradingCost, { marketType, tradePlanType, executionModel });
  const inverse = backtestStrategy(candles, inverseStrategy, interval, tradingCost, { marketType, tradePlanType, executionModel });
  const recommendation = classifyInverseComparison({ original, inverse, minTrades, tradingCost });
  return {
    assetSampleStart: candles?.[0]?.openTime ?? null,
    assetSampleEnd: candles?.at(-1)?.openTime ?? null,
    strategyId: strategy.id,
    strategyName: strategy.name,
    direction: strategy.direction,
    inverseStrategyId: inverseStrategy.id,
    inverseDirection: inverseStrategy.direction,
    interval,
    tradingCost,
    minTrades,
    original,
    inverse,
    deltas: summarizeInverseDeltas(original, inverse),
    recommendation
  };
}
...
export function getCurrentSignal(candles, strategy, interval) {
  ...
  const signalCandleOpenTime = candles[latestIndex].openTime;
  const signalCandleCloseTime = signalCandleOpenTime + intervalHours(interval) * 3600 * 1000;
  return {
    ...
    validUntil: signalCandleCloseTime + validMs(strategy)
  };
}
...
export function scoreFuturesSentiment({ checks, sentiment, wantsLong, wantsShort }) {
  let score = 0;
  const accountRatio = sentiment?.accounts?.ratio;
  const topRatio = sentiment?.topPositions?.ratio;

  if (Number.isFinite(accountRatio)) {
    if ((wantsLong && accountRatio > 2.2) || (wantsShort && accountRatio < 0.45)) {
      checks.push(["多空拥挤", "风险", `普通账户多空比 ${accountRatio.toFixed(2)}，同向过于拥挤`]);
      score -= 3;
    } else if ((wantsLong && accountRatio >= 0.8) || (wantsShort && accountRatio <= 1.2)) {
      checks.push(["多空结构", "通过", `普通账户多空比 ${accountRatio.toFixed(2)}，未见明显反向拥挤`]);
      score += 2;
    } else {
      checks.push(["多空结构", "一般", `普通账户多空比 ${accountRatio.toFixed(2)}，方向配合一般`]);
    }
  } else {
    checks.push(["多空结构", "缺失", "未获取到普通账户多空比"]);
  }

  if (Number.isFinite(topRatio)) {
    if ((wantsLong && topRatio > 2.5) || (wantsShort && topRatio < 0.4)) {
      checks.push(["大户倾向", "风险", `顶级交易员持仓多空比 ${topRatio.toFixed(2)}，可能过度拥挤`]);
      score -= 2;
    } else if ((wantsLong && topRatio >= 1.05) || (wantsShort && topRatio <= 0.95)) {
      checks.push(["大户倾向", "通过", `顶级交易员持仓多空比 ${topRatio.toFixed(2)}，与方向较一致`]);
      score += 3;
    } else {
      checks.push(["大户倾向", "一般", `顶级交易员持仓多空比 ${topRatio.toFixed(2)}，方向支持不强`]);
    }
  } else {
    checks.push(["大户倾向", "缺失", "未获取到顶级交易员持仓多空比"]);
  }

  return score;
}

export function evaluateFuturesFilters({ candles, benchmarkCandles, funding, openInterest, sentiment, direction }) {
  ...
  const directionText = String(direction || "");
  const wantsLong = directionText.includes("多") || directionText.toUpperCase().includes("LONG");
  const wantsShort = directionText.includes("空") || directionText.toUpperCase().includes("SHORT");

  if (funding && Number.isFinite(funding.fundingRate)) {
    const rate = funding.fundingRate;
    if (wantsLong && rate > 0.0008) {
      checks.push(["资金费率", "风险", `资金费率偏高 ${(rate * 100).toFixed(4)}%，多头可能拥挤`]);
    } else if (wantsShort && rate < -0.0008) {
  ...
  score += scoreFuturesSentiment({ checks, sentiment, wantsLong, wantsShort });
  return {
    score: clamp(score, 0, 25),
    liquidityScore,
    checks,
    futuresContext: { funding, openInterest, sentiment }
  };
}
...
function classifyInverseComparison({ original, inverse, minTrades, tradingCost }) {
  if (!inverse || inverse.trades < minTrades) return "insufficient_inverse_sample";
  if (!original || original.trades < minTrades) return "insufficient_original_sample";
  const inverseOutOfSampleReturn = inverse.outOfSample?.totalReturn ?? 0;
  const inverseAverage = inverse.averageReturn ?? 0;
  const originalReturn = original.totalReturn ?? 0;
  const inverseReturn = inverse.totalReturn ?? 0;
  if (
    originalReturn < 0 &&
    inverseReturn > 0 &&
    inverseOutOfSampleReturn > 0 &&
    inverseAverage > tradingCost
  ) {
    return "inverse_candidate";
  }
  if (inverseReturn > originalReturn && inverseOutOfSampleReturn > 0) return "inverse_watch";
  return "keep_original_or_reject";
}

function summarizeInverseDeltas(original, inverse) {
  if (!original || !inverse) return null;
  return {
    totalReturn: inverse.totalReturn - original.totalReturn,
    cagr: inverse.cagr - original.cagr,
    winRate: inverse.winRate - original.winRate,
    averageReturn: inverse.averageReturn - original.averageReturn,
    maxDrawdown: inverse.maxDrawdown - original.maxDrawdown,
    outOfSampleTotalReturn: (inverse.outOfSample?.totalReturn ?? 0) - (original.outOfSample?.totalReturn ?? 0)
  };
}
