...
export function compareStrategyInversion({ candles, strategy, interval, tradingCost, minTrades = CONFIG.minTrades }) {
  const inverseStrategy = invertStrategyDirection(strategy);
  const original = backtestStrategy(candles, strategy, interval, tradingCost);
  const inverse = backtestStrategy(candles, inverseStrategy, interval, tradingCost);
