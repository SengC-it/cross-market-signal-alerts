import { fetchHistoricalFunding } from "./market-data/funding-history.js";
import { fetchBinanceExchangeFilters } from "./market-data/exchange-info.js";
...
export async function getCryptoCandlesRange(symbol, interval, startTime, endTime, limit = 1000) {
  ...
  const response = await fetchWithTimeout(`${BINANCE_REST}/klines?${params}`);
  if (!response.ok) {
    if (response.status === 451) throw new Error(`Binance spot restricted by location: ${symbol} ${interval} 451`);
    throw new Error(`Binance spot historical kline failed: ${symbol} ${interval} ${response.status}`);
  }
  const data = await response.json();
  return data.map(mapBinanceKline);
}
...
export async function getFutures24hTickers() {
  const response = await fetchWithTimeout(`${BINANCE_FUTURES_REST}/ticker/24hr`);
  if (!response.ok) {
    if (response.status === 451) throw new Error("Binance futures 24h ticker restricted by location: 451");
    throw new Error(`Binance futures 24h ticker failed: ${response.status}`);
  }
  const data = await response.json();
  return data.map((item) => ({
    symbol: item.symbol,
...
export async function getFuturesExchangeSymbols() {
  const response = await fetchWithTimeout(`${BINANCE_FUTURES_REST}/exchangeInfo`);
  if (!response.ok) {
    if (response.status === 451) throw new Error("Binance futures exchange info restricted by location: 451");
    throw new Error(`Binance futures exchange info failed: ${response.status}`);
  }
  const data = await response.json();
  return (data.symbols || [])
    .filter((item) =>
...
export async function getFuturesOrderBook(symbol, limit = 100) {
  let response;
  try {
    response = await fetchWithTimeout(`${BINANCE_FUTURES_REST}/depth?symbol=${symbol}&limit=${limit}`);
  } catch {
    return null;
  }
  if (!response.ok) return null;
...
export async function getFuturesCandlesRange(symbol, interval, startTime, endTime, limit = 1500) {
  const params = new URLSearchParams({
    symbol,
    interval,
    startTime: String(Number(startTime)),
    endTime: String(Number(endTime)),
    limit: String(Math.min(1500, Math.max(1, Number(limit) || 1500)))
  });
  const response = await fetchWithTimeout(`${BINANCE_FUTURES_REST}/klines?${params}`);
  if (!response.ok) {
    if (response.status === 451) throw new Error(`Binance futures restricted by location: ${symbol} ${interval} 451`);
    throw new Error(`Binance futures historical kline failed: ${symbol} ${interval} ${response.status}`);
  }
  const data = await response.json();
  return data.map(mapBinanceKline);
}
...
export async function getFuturesFundingHistory(symbol, startTime, endTime) {
  ...
  return result.events.map((event) => ({
    ...
    fundingTime: event.time
  }));
}

export async function getFuturesFundingHistoryWithCoverage(symbol, startTime, endTime) {
  return fetchHistoricalFunding({
    symbol,
    startTime,
    endTime,
    fetchImpl: fetchWithTimeout
...
export async function getFuturesExchangeFilters(symbol) {
  return fetchBinanceExchangeFilters({
    symbol,
    fetchImpl: fetchWithTimeout
  });
}
