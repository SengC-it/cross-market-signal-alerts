...
export async function getFuturesFundingHistory(symbol, startTime, endTime) {
  ...
  const response = await fetchWithTimeout(`${BINANCE_FUTURES_REST}/fundingRate?${params}`);
  if (!response.ok) {
    if (response.status === 451) throw new Error(`Binance futures funding restricted by location: ${symbol} 451`);
    throw new Error(`Binance futures funding history failed: ${symbol} ${response.status}`);
  }
  const data = await response.json();
