...
} from "./strategy-display-groups.js";
...
export function isV42ForwardSignal(signal = {}) {
  return strategyGroupForSignal(signal)?.id === "v4_2_forward";
}
...
export function buildForwardStrategyPerformance({
  ...
  for (const definition of STRATEGY_DISPLAY_GROUPS) {
    const metadata = {
      id: definition.id,
      label: definition.label,
      order: definition.order,
      scope: strategyScope(definition.id)
    };
    if (["legacy_production", "v4_2_forward", "v4_forward", "v4_shadow"].includes(definition.id)) {
      const groupSignals = signals.filter((signal) =>
        !isPaperSignal(signal)
        && strategyGroupForSignal(signal)?.id === definition.id
      );
      performance[definition.key] = summarizeSignalStrategy(groupSignals, metadata);
      continue;
    }

    performance[definition.key] = summarizePaperStrategy(
      paperModelRuns,
      { ...metadata, modelId: paperModelId(definition.id) }
    );
  }
...
function strategyScope(groupId) {
  if (groupId === "legacy_production") return "旧 scanner 实际历史邮件";
  if (groupId === "v4_forward") return "dynamic_relative_strength_breakout / current V4 production forward";
  if (groupId === "v4_shadow") return "dynamic_relative_weakness_breakdown / SHADOW_ONLY";
  if (groupId === "v4_2_forward") return "dynamic_relative_strength_breakout / STRONG_EXTENSION_10_15";
  return paperModelId(groupId);
}

function paperModelId(groupId) {
  return {
...
function summarizePaperStrategy(runs, metadata) {
  ...
  for (const run of runs) {
    if (
      strategyGroupForPaperRun(run)?.id !== metadata.id
      || run?.model_id !== metadata.modelId
