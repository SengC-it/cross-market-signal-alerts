  ...
  return {
    ...
    crossSectionalUniverseAt(time, lookbackHours = 24) {
      ...
      for (const asset of activeAssets) {
        ...
        if (assetShouldHaveDataAt({ asset, time: timestamp, dataset, universeMetadata })) {
...
function buildFundingCrowdingSignals({ context, definition }) {
  ...
  for (const fundingEventTime of eventTimes) {
    ...
    for (const dataset of context.datasets) {
      ...
      if (!current || !context.assetsAt(current.openTime).has(dataset.asset)) continue;
      const availableAt = Number(current.openTime) + intervalMilliseconds("1h");
      if (availableAt > toTimestamp(context.window.endExclusive)) continue;
      const eightHoursAgo = candleAt(dataset, Number(current.openTime) - 8 * 3600 * 1000);
      if (!eightHoursAgo || funding.time >= availableAt) continue;
      const return8h = priceReturn(current.close, eightHoursAgo.close);
      if (!Number.isFinite(return8h)) continue;
      rows.push({ dataset, current, funding, return8h, availableAt });
    }
