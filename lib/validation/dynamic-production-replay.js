...
export function replayDynamicProductionSignals({
  ...
  dynamicSpotAssets = [],
  selected = null,
  includePipelineDiagnostics = false,
  includeOpportunityPassedSignals = false
} = {}) {
  ...
  const pipelineDiagnostics = includePipelineDiagnostics
  ...
  for (const signalAvailableAt of timestamps) {
    if (pipelineDiagnostics) pipelineDiagnostics.timestampCount++;
    ...
    if (pipelineDiagnostics) {
      pipelineDiagnostics.visibleTickerObservations += visibleRows.length;
    }
    ...
    for (const dataset of normalizedDatasets) {
      ...
      for (const selectedStrategyId of strategyIds) {
        ...
        if (Number.isFinite(lastSignalAt)
          ...
          if (pipelineDiagnostics) recordDiagnosticReason(pipelineDiagnostics.cooldownReasons, "cooldown");
          continue;
        }
        if (pipelineDiagnostics) pipelineDiagnostics.strategyEvaluated++;
        ...
        const opportunityPassed = evaluation.opportunity?.passed === true;
        const signalAllowed = evaluation.scoreGatePassed
          || (includeOpportunityPassedSignals && opportunityPassed);
        if (!signalAllowed) {
          if (pipelineDiagnostics) {
            recordDiagnosticReason(
              pipelineDiagnostics.strategyGateRejections,
              classifyStrategyRejection(evaluation)
            );
          }
          continue;
        }
        if (pipelineDiagnostics && evaluation.scoreGatePassed) pipelineDiagnostics.scoreGatePassed++;
        ...
        signals.push({
          ...
          scoreGatePassed: evaluation.scoreGatePassed,
          opportunityPassed,
          signalSelectionMode: includeOpportunityPassedSignals
            ? "OPPORTUNITY_PASSED_SCORE_INDEPENDENT_RESEARCH"
            : "FROZEN_SCORE_GATE",
          details: {
            ...evaluation.features,
        ...
        if (pipelineDiagnostics) {
          pipelineDiagnostics.replaySignals++;
          if (quality.primaryEligible) pipelineDiagnostics.primaryEligible++;
          for (const reason of quality.exclusionReasons || []) {
            recordDiagnosticReason(pipelineDiagnostics.signalQualityExclusions, reason);
          }
        }
  ...
  if (pipelineDiagnostics) {
    result.pipelineDiagnostics = finalizePipelineDiagnostics(pipelineDiagnostics);
  }
  return result;
}
...
function createPipelineDiagnostics() {
  return {
    ...
    replaySignals: 0,
    primaryEligible: 0,
    poolCandidateRejections: {},
    poolRankRejections: {},
    cooldownReasons: {},
    strategyGateRejections: {},
    signalQualityExclusions: {}
  };
}

function recordPoolDiagnostics({
  diagnostics,
  strategyId,
  visibleRows,
  universeAssets,
  ...
  const isStrong = strategyId === "dynamic_relative_strength_breakout";
  const direction = isStrong ? "strong" : "weak";
  const selectedPool = new Set(isStrong ? strongPool : weakPool);
  const existing = isStrong
    ? excludedAssets
    : new Set([...excludedAssets, ...strongPool]);
  const candidatePredicate = isStrong
    ? isDynamicStrongPoolCandidate
    : isDynamicWeakPoolCandidate;

  for (const row of visibleRows) {
    if (!universeAssets.has(row.symbol)) continue;
    const ticker = row.ticker;
    if (candidatePredicate(ticker, existing, knownFuturesSymbols)) {
      diagnostics.poolCandidates++;
      if (selectedPool.has(row.symbol)) diagnostics.poolSelected++;
      else recordDiagnosticReason(diagnostics.poolRankRejections, "pool_rank_cutoff");
      continue;
    }
    recordDiagnosticReason(
      diagnostics.poolCandidateRejections,
      classifyPoolRejection(ticker, direction, existing, knownFuturesSymbols)
    );
  }
}

function classifyPoolRejection(ticker = {}, direction, existing, futuresSymbols) {
  const symbol = String(ticker.symbol || "");
  if (!symbol.endsWith("USDT")) return "invalid_symbol";
  if (existing?.has(symbol)) return "existing_asset";
  if (futuresSymbols && !futuresSymbols.has(symbol)) return "not_futures_symbol";
  if (CONFIG.dynamicSpotPoolExcludedPatterns.some((pattern) => symbol.includes(pattern))) {
    return "excluded_symbol_pattern";
  }
  const priceChangePercent = Number(ticker.priceChangePercent);
  if (!Number.isFinite(priceChangePercent)) return "missing_momentum";
  if (direction === "strong") {
    if (priceChangePercent < CONFIG.dynamicSpotPoolMinPriceChangePercent) return "momentum_too_low";
    if (priceChangePercent > CONFIG.dynamicSpotPoolMaxPriceChangePercent) return "momentum_too_high";
  } else {
    if (priceChangePercent < CONFIG.dynamicWeakSpotPoolMinPriceChangePercent) return "momentum_too_low";
    if (priceChangePercent > CONFIG.dynamicWeakSpotPoolMaxPriceChangePercent) return "momentum_too_high";
  }
  if (!Number.isFinite(Number(ticker.quoteVolume))) return "missing_quote_volume";
  const minQuoteVolume = direction === "strong"
    ? CONFIG.dynamicSpotPoolMinQuoteVolume
    : CONFIG.dynamicWeakSpotPoolMinQuoteVolume;
  if (Number(ticker.quoteVolume) < minQuoteVolume) return "quote_volume_too_low";
  return "other";
}

function classifyStrategyRejection(evaluation = {}) {
  if (evaluation.opportunity?.passed === true && evaluation.scoreGatePassed !== true) {
    return "score_below_threshold";
  }
  const reason = String(evaluation.reason || evaluation.opportunity?.reason || "other");
  if (reason === "insufficient_momentum") return "momentum_too_low";
  if (reason === "overheated_momentum") return "momentum_too_high";
  if (reason === "insufficient_relative_strength") return "relative_strength_insufficient";
  if (reason === "no_breakout") return "no_breakout";
  if (reason === "insufficient_volume") return "volume_multiple_low";
  if (reason === "weak_edge_volume" || reason === "overheated_volume") return "volume_multiple_high";
  if (reason === "recommendation_score_below_floor") return "score_below_threshold";
  if (reason === "weakness_gate") return "weakness_gate";
  return reason;
}

function finalizePipelineDiagnostics(diagnostics) {
  return {
    ...diagnostics,
    poolCandidateRejections: sortCountMap(diagnostics.poolCandidateRejections),
    poolRankRejections: sortCountMap(diagnostics.poolRankRejections),
    cooldownReasons: sortCountMap(diagnostics.cooldownReasons),
    strategyGateRejections: sortCountMap(diagnostics.strategyGateRejections),
    signalQualityExclusions: sortCountMap(diagnostics.signalQualityExclusions)
  };
}

function sortCountMap(value = {}) {
  return Object.fromEntries(Object.entries(value).sort((left, right) => {
    const countOrder = Number(right[1]) - Number(left[1]);
    return countOrder || left[0].localeCompare(right[0]);
  }));
}

function recordDiagnosticReason(map, reason) {
  ...
  return (Array.isArray(datasets) ? datasets : [])
    ...
    .map((dataset) => {
      ...
      for (let index = 0; index < candles.length; index++) {
        ...
        const diagnosticQuoteVolume = diagnosticQuoteVolumeForCandle(candles[index]);
        if (Number.isFinite(diagnosticQuoteVolume)) rollingQuoteVolume += diagnosticQuoteVolume;
        if (!Number.isFinite(quoteVolume)) rollingInvalidVolume++;
        if (index >= 24) {
          const outgoing = quoteVolumeForCandle(candles[index - 24]);
          const diagnosticOutgoing = diagnosticQuoteVolumeForCandle(candles[index - 24]);
          if (Number.isFinite(diagnosticOutgoing)) rollingQuoteVolume -= diagnosticOutgoing;
          if (!Number.isFinite(outgoing)) rollingInvalidVolume--;
        }
        replayQuoteVolumes.push(index >= 23 && Number.isFinite(rollingQuoteVolume)
          ? rollingQuoteVolume
          : null);
...
function quoteVolumeForCandle(candle) {
  ...
  return Number.isFinite(quoteVolume) ? quoteVolume : null;
}

function diagnosticQuoteVolumeForCandle(candle) {
