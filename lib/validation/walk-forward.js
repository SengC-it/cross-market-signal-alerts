...
export function boundBacktestOptionsToEnd(backtestOptions = {}, testEnd) {
  ...
  const boundedFundingEvents = filterTimestampedRows(originalFundingEvents, end, "time");
  ...
  if (backtestOptions.fundingCoverage || originalExecutionModel.fundingCoverage) {
...
function clipFundingCoverage(coverage, events, testEnd) {
  ...
  const coverageCanBeComplete = coverage?.complete === true
    ...
    && originalEnd >= testEnd
    && (!Number.isFinite(originalCoverageEnd) || originalCoverageEnd >= testEnd);
  const gaps = (Array.isArray(coverage?.gaps) ? coverage.gaps : [])
    .map((gap) => ({
      ...
      start: toTimestamp(gap?.start ?? gap?.requestedStart),
        ...
        testEnd,
        toTimestamp(gap?.end ?? gap?.requestedEnd) ?? testEnd
      )
    }))
    .filter((gap) => Number.isFinite(gap.start) && gap.start < testEnd);
  return buildFundingCoverage({
    requestedStart,
    requestedEnd,
    events,
    complete: coverageCanBeComplete && gaps.length === 0,
    gaps,
