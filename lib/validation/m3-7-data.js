...
export async function prepareM37Data({
  ...
  const providerGapRegistry = normalizeProviderGapRegistry(
    input.providerGapRegistry || M37_PROVIDER_GAP_REGISTRY
  );
  ...
  const coverageByFamily = Object.fromEntries(definitions.map((definition) => {
    ...
    return [definition.id, {
      ...
      providerGapPurgedSignals: Number(signalContext.providerGapEvaluation.byFamily?.[definition.id]?.providerGapPurgedSignals) || 0,
  ...
  const signalRelevantFundingCoverage = coverageStatus(
    Object.values(coverageByFamily).every((coverage) => coverage.signalRelevantFundingCoverage.complete === true),
    true
  );
  const signalRelevantLowerTfCoverage = coverageStatus(
  ...
  const providerGapPolicy = summarizeM37ProviderGapPolicy({
    ...
    inputCoverage: {
      ...
      signalRelevantFundingCoverage,
      signalRelevantLowerTfCoverage
    }
  });
  ...
  const index = {
    ...
    providerGapPolicyVersion: M37_PROVIDER_GAP_POLICY_VERSION,
    providerGapRegistry,
    providerGapPolicy,
...
export async function loadM37Data({ dataDir = ".local/m3-7-data", includeLowerTimeframe = false } = {}) {
  const root = resolve(dataDir);
  if (!existsSync(join(root, "index.json"))) throw new Error("M3_7_DATA_REQUIRED");
  const index = JSON.parse(await readFile(join(root, "index.json"), "utf8"));
  const datasetDescriptors = Array.isArray(index.datasets) ? index.datasets : [];
  const datasets = [];
  for (const descriptor of datasetDescriptors) {
    const dataset = await loadM37Dataset({ dataDir: root, descriptor });
    if (includeLowerTimeframe) {
    ...
    datasets.push(lightweightDataset);
  }
  const benchmarkCandles = Array.isArray(index.benchmarkCandles)
    ...
    : existsSync(resolve(root, index.benchmarkPath || ""))
      ...
      : [];
  return { ...index, dataDir: root, datasetDescriptors, datasets, benchmarkCandles };
}

export async function loadM37Dataset({ dataDir = ".local/m3-7-data", descriptor } = {}) {
  const root = resolve(dataDir);
  const path = resolve(root, descriptor?.path || "");
  if (!descriptor?.path || !existsSync(path)) throw new Error(`M3_7_DATA_REQUIRED: ${descriptor?.path || "dataset"}`);
