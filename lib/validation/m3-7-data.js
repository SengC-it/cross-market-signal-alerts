...
export async function prepareM37Data({
  ...
  const spansByFamily = buildFamilySpans(signalsByFamily);
  const allSpansByAsset = mergeSpansByAsset(Object.values(spansByFamily));
  const lowerResults = new Map();
  ...
  await mapConcurrent(assets, concurrency, async (asset, index) => {
    ...
    lowerResults.set(asset, lower);
    onProgress({ phase: "5m", asset, index: index + 1, total: assets.length, status: lower.coverage.status });
  });

  const finalDatasets = datasetsWithFunding.map((dataset) => {
    const lower = lowerResults.get(dataset.asset) || emptyLowerResult();
    const lowerByFamily = Object.fromEntries(definitions.map((definition) => {
      const familySpans = spansByFamily[definition.id].get(dataset.asset) || [];
  ...
  for (const dataset of finalDatasets) {
    const descriptor = { asset: dataset.asset, path: `datasets/${dataset.asset}.json` };
    await writeJson(join(outputRoot, descriptor.path), dataset);
  ...
  const metadata = Array.isArray(input.historicalUniverseMetadata)
    ...
    : [];
  const partialLifecycleAssetsIncluded = finalDatasets.filter((dataset) => {
  ...
  const coverageByFamily = Object.fromEntries(definitions.map((definition) => {
    const lower = finalDatasets.map((dataset) => dataset.lowerTimeframeCoverage?.byFamily?.[definition.id]);
    const lowerComplete = lower.length > 0 && lower.every((value) => value?.complete === true || value?.status === "NOT_REQUIRED");
    const fundingComplete = finalDatasets.length > 0 && finalDatasets.every((dataset) => {
    ...
    return [definition.id, {
      ...
      signalRelevantFundingCoverage: coverageStatus(fundingComplete, finalDatasets.length > 0),
      requestedLowerSpans: sumSpanCount(spansByFamily[definition.id]),
      fundingEventsAvailable: finalDatasets.reduce((sum, dataset) => sum + dataset.fundingEvents.length, 0)
    }];
  }));
  const requiredHistoricalFundingAvailable = finalDatasets.length > 0
    && finalDatasets.every((dataset) => dataset.fundingCoverage?.complete === true);
  ...
  const dataCoverage = {
    ...
    researchDatasetAssetCount: finalDatasets.length,
  ...
  return { index, datasets: finalDatasets, signalsByFamily, spansByFamily };
}
...
function buildFamilySpans(signalsByFamily) {
  return Object.fromEntries(M37_FAMILY_DEFINITIONS.map((definition) => {
    const spans = new Map();
    for (const signal of signalsByFamily[definition.id] || []) {
      const start = Number(signal.entryEligibleAt);
      const end = start + Number(definition.execution.holdHours) * HOUR_MS;
