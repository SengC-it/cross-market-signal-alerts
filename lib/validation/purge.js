...
export function classifyOosBoundary(record, { testStart, testEnd } = {}) {
  ...
  const hasFrozenSpec = hasFrozenTradeSpec(record, source);
  ...
  if (!hasFrozenSpec || isInvalidTradePlan(record)) {
    return {
      status: NOT_APPLICABLE,
      eligible: false,
      inTestWindow: isSignalInTestWindow(record, { testStart: start, testEnd: end }),
      reasons: ["no_frozen_trade_spec"],
      signalAvailableAt,
      entryEligibleAt,
      maxHoldingTime
    };
...
export function partitionByOosBoundary(records = [], bounds = {}) {
  ...
  for (const record of Array.isArray(records) ? records : []) {
    ...
    else if (classification.status === NOT_APPLICABLE) notApplicable.push(classified);
    else outsideWindow.push(classified);
  }
  return { eligible, purged, outsideWindow, notApplicable };
}
...
export function isSignalInTestWindow(record, { testStart, testEnd } = {}) {
  const start = toTimestamp(testStart);
  const end = toTimestamp(testEnd);
  const signalTime = toTimestamp(
    record?.signalAvailableAt
      ?? record?.tradeSpec?.signalAvailableAt
      ?? record?.signalCandleOpenTime
      ?? record?.tradeSpec?.signalCandleOpenTime
  );
...
function hasFrozenTradeSpec(record, source) {
  const valuesPresent = Number.isFinite(toTimestamp(source?.signalAvailableAt))
    && Number.isFinite(toTimestamp(source?.entryEligibleAt))
    && Number.isFinite(toTimestamp(source?.maxHoldingTime))
    && Number.isFinite(Number(source?.referencePrice))
    && Number.isFinite(Number(source?.stopLoss))
    && Number.isFinite(Number(source?.takeProfit));
  if (!valuesPresent) return false;
  return toTimestamp(source.signalAvailableAt) <= toTimestamp(source.entryEligibleAt)
    && toTimestamp(source.entryEligibleAt) <= toTimestamp(source.maxHoldingTime);
}

function isInvalidTradePlan(record) {
  return (record?.status === "NO_ENTRY" || record?.status === "MISSED_ENTRY")
    && record?.reason === "invalid_trade_plan";
}
