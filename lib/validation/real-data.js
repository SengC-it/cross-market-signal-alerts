...
import { createExecutionModel } from "../backtest/execution-model.js";
...
export const M3_REAL_MANIFEST_SHA256 =
  "6431787daf657f0d861b367fa549d7d71582e37b381448f995a746b13dd1e08c";
...
export const PROVIDER_CHECKSUM_STATUS = Object.freeze({
  VERIFIED: "VERIFIED",
...
export function summarizeExecutionCostModel({ executionModel = {}, marketType = "futures" } = {}) {
  const model = createExecutionModel({ marketType, ...executionModel });
  const roundTripFeePct = Number.isFinite(Number(model.legacyRoundTripCostPct))
    ? Number(model.legacyRoundTripCostPct)
    : Number(model.entryFeePct) + Number(model.exitFeePct);
  return {
    source: Number.isFinite(Number(model.legacyRoundTripCostPct))
      ? "CONFIG.futuresTradingCost"
      : "executionModel",
    roundTripFeePct,
    entryFeePct: Number(model.entryFeePct),
    exitFeePct: Number(model.exitFeePct),
    spreadPct: Number(model.spreadPct),
    entrySlippagePct: Number(model.entrySlippagePct),
    exitSlippagePct: Number(model.exitSlippagePct)
  };
}

export function verifyBinanceVisionChecksum({ bytes, checksumText } = {}) {
  if (checksumText == null || String(checksumText).trim() === "") {
    return { status: PROVIDER_CHECKSUM_STATUS.CHECKSUM_UNAVAILABLE, expectedSha256: null, actualSha256: null };
  }
  const expectedSha256 = String(checksumText).match(/\b[a-f0-9]{64}\b/i)?.[0]?.toLowerCase() || null;
  if (!expectedSha256) {
    return { status: PROVIDER_CHECKSUM_STATUS.CHECKSUM_UNAVAILABLE, expectedSha256: null, actualSha256: null };
  }
  const actualSha256 = createHash("sha256").update(bytes).digest("hex");
  return {
    status: actualSha256 === expectedSha256
...
export function summarizeProviderChecksum(state = {}) {
  const verifiedFiles = Number(state.verifiedFiles) || 0;
  const unavailableFiles = Number(state.unavailableFiles) || 0;
  const mismatchedFiles = Number(state.mismatchedFiles) || 0;
  return {
    verifiedFiles,
    unavailableFiles,
    mismatchedFiles,
    status: mismatchedFiles > 0
      ? PROVIDER_CHECKSUM_STATUS.MISMATCH
      : verifiedFiles > 0 && unavailableFiles === 0
        ? PROVIDER_CHECKSUM_STATUS.VERIFIED
...
export async function verifyFrozenM3Data({
  ...
  const manifestSha256 = await sha256CanonicalTextFile(manifestFile);
  ...
  if (manifest.providerChecksum?.mismatchedFiles > 0
    || index.providerChecksum?.mismatchedFiles > 0
    || manifest.providerChecksum?.status === PROVIDER_CHECKSUM_STATUS.MISMATCH
    || index.providerChecksum?.status === PROVIDER_CHECKSUM_STATUS.MISMATCH) {
    throw new Error("M3_DATA_HASH_MISMATCH: provider-checksum");
  }
  if (manifest.providerChecksum && index.providerChecksum
    && !sameProviderChecksum(manifest.providerChecksum, index.providerChecksum)) {
    throw new Error("M3_DATA_HASH_MISMATCH: provider-checksum-index");
  }
...
export async function prepareM3RealData({
  ...
  const executionCostModel = summarizeExecutionCostModel({
    executionModel: effectiveExecutionModel,
    marketType: "futures"
  });
  const providerChecksumState = { verifiedFiles: 0, unavailableFiles: 0, mismatchedFiles: 0 };
  ...
  const providerChecksum = summarizeProviderChecksum(providerChecksumState);
  const index = {
    datasetId: "binance_usdm_m3_2025_08_2026_08",
    dataSource: "BINANCE_VISION_REAL_DATA",
    windowStart: window.start,
    ...
    checksums: {
      algorithm: "SHA-256",
      files: dataChecksums,
      providerChecksumsVerified: providerChecksum.status === PROVIDER_CHECKSUM_STATUS.VERIFIED,
      providerChecksumReason: providerChecksum.status
    },
...
async function fetchSeriesForSymbols({ symbols, interval, startTime, endTime, market, dataDir, concurrency, providerChecksumState, onProgress }) {
  const result = new Map();
  await mapConcurrent(symbols, concurrency, async (symbol, index) => {
    try {
      const series = await fetchVisionSeries({ symbol, interval, startTime, endTime, market, dataDir, providerChecksumState });
...
async function fetchVisionSeries({ symbol, interval, startTime, endTime, market, dataDir, providerChecksumState = null }) {
  ...
  for (const month of monthKeys(startTime, endTime)) {
    ...
    if (existsSync(cachePath)) {
      ...
      recordProviderChecksum(providerChecksumState, PROVIDER_CHECKSUM_STATUS.CHECKSUM_UNAVAILABLE);
      continue;
    }
    ...
    const checksum = await fetchVisionChecksum({ url, bytes: zip });
    recordProviderChecksum(providerChecksumState, checksum.status);
    if (checksum.status === PROVIDER_CHECKSUM_STATUS.MISMATCH) {
      throw new Error(`M3_DATA_HASH_MISMATCH: provider-checksum:${url}`);
    }
...
async function fetchVisionChecksum({ url, bytes }) {
  try {
    const response = await fetchWithTimeout(`${url}.CHECKSUM`);
    if (!response.ok || response.status === 404) {
      return { status: PROVIDER_CHECKSUM_STATUS.CHECKSUM_UNAVAILABLE };
    }
    return verifyBinanceVisionChecksum({
      bytes,
...
function recordProviderChecksum(state, status) {
  if (!state) return;
  if (status === PROVIDER_CHECKSUM_STATUS.VERIFIED) state.verifiedFiles++;
  else if (status === PROVIDER_CHECKSUM_STATUS.MISMATCH) state.mismatchedFiles++;
  else state.unavailableFiles++;
}
...
function assertFrozenMetadata({ index, manifest }) {
  ...
  const executionCostIsValid = Number(manifest?.executionCostModel?.roundTripFeePct) === CONFIG.futuresTradingCost
    && Number(index?.executionCostModel?.roundTripFeePct) === CONFIG.futuresTradingCost
    && Number(manifest?.executionCostModel?.entryFeePct) === CONFIG.futuresTradingCost / 2
    && Number(manifest?.executionCostModel?.exitFeePct) === CONFIG.futuresTradingCost / 2;
  if (!windowIsValid
    || !intervalsAreValid
    || !historicalUniverseIsValid
    || !executionCostIsValid
    || !index?.datasetId
...
function sameProviderChecksum(left, right) {
  return Number(left?.verifiedFiles) === Number(right?.verifiedFiles)
    && Number(left?.unavailableFiles) === Number(right?.unavailableFiles)
    && Number(left?.mismatchedFiles) === Number(right?.mismatchedFiles)
...
export async function sha256CanonicalTextFile(path) {
