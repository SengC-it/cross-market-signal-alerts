...
function buildAttritionFunnel({ validation, signals, pipelineDiagnostics }) {
  ...
  const holdoutRawSignals = numberOr(holdoutMetrics.rawSignals, 0);
  const initialTrainingContextSignals = signals.length - rawOosSignals - holdoutRawSignals;
  const conservationTotal = initialTrainingContextSignals + rawOosSignals + holdoutRawSignals;
  const conservation = {
    replaySignals: signals.length,
    initialTrainingContextSignals,
    developmentOosSignals: rawOosSignals,
    holdoutSignals: holdoutRawSignals,
    sum: conservationTotal,
    valid: Number.isInteger(initialTrainingContextSignals)
      && initialTrainingContextSignals >= 0
      && conservationTotal === signals.length
  };
  ...
  const stages = [
    ...
    stage("initial_training_context_not_oos", initialTrainingContextSignals, signals.length, "replay_signals", {
      not_attributed_to_oos: initialTrainingContextSignals
    }),
    stage("development_oos", rawOosSignals, signals.length, "replay_signals", {}),
    stage("holdout", holdoutRawSignals, signals.length, "replay_signals", {
      not_attributed_to_development: holdoutRawSignals
    }),
...
function buildExitDecomposition(trades) {
  ...
  return keys.map((key) => {
    ...
    return {
      ...
      exitReason: key,
      ...groupSummary(group),
      averageHoldingHours: average(group.map((trade) => Number(trade.holdingHours)))
    };
  });
}
