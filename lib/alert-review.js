...
export function reviewAlertWithCandles(alert, candles, now = Date.now()) {
  ...
  const windowCandles = candles
    .filter((candle) => candle.openTime > reviewStartTime && candle.openTime + intervalMs <= now)
    .sort((a, b) => a.openTime - b.openTime);
  if (!windowCandles.length) return pendingReview("等待止盈止损K线", reviewStartTime + intervalMs);
