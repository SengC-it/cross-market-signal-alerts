...
export async function sendEmail({ subject, text, idempotencyKey }) {
  if (process.env.GMAIL_SMTP_USER && process.env.GMAIL_APP_PASSWORD) {
    return sendWithGmailSmtp({ subject, text, idempotencyKey });
  }
  if (process.env.RESEND_API_KEY) {
    return sendWithResend({ subject, text, idempotencyKey });
  }
  if (process.env.SENDGRID_API_KEY) {
    return sendWithSendGrid({ subject, text, idempotencyKey });
  }
  console.log("Email provider is not configured. Would send:", { subject, text });
  return { skipped: true, reason: "missing_email_provider" };
}

async function sendWithGmailSmtp({ subject, text, idempotencyKey }) {
  ...
  return transporter.sendMail({
    ...
    subject,
    text,
    headers: idempotencyKey ? { "X-Signal-Idempotency-Key": idempotencyKey } : undefined
  });
}

async function sendWithResend({ subject, text, idempotencyKey }) {
...
async function sendWithSendGrid({ subject, text, idempotencyKey }) {
  const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
    ...
    body: JSON.stringify({
      ...
      ...(idempotencyKey ? { custom_args: { signal_idempotency_key: idempotencyKey } } : {})
    })
  });
  if (!response.ok) {
