...
export async function recordRunLog(log) {
  ...
  if (response.status === 400 && (body.includes("email_status") || body.includes("warnings"))) {
    const legacyLog = { ...log };
    if (body.includes("email_status")) delete legacyLog.email_status;
    if (body.includes("warnings")) delete legacyLog.warnings;
...
export async function fetchRecentRunLogs(limit = 50) {
  if (!isSupabaseConfigured()) return [];
  const response = await fetchRunLogs(limit, "id,created_at,started_at,finished_at,scan_group,candidates_count,signals_count,emailed,email_status,errors,warnings");
  if (response.ok) return response.json();

  const body = await response.text();
  if (response.status === 400 && (body.includes("email_status") || body.includes("warnings"))) {
    const select = body.includes("email_status")
      ? "id,created_at,started_at,finished_at,scan_group,candidates_count,signals_count,emailed,errors"
      : "id,created_at,started_at,finished_at,scan_group,candidates_count,signals_count,emailed,email_status,errors";
    const retry = await fetchRunLogs(limit, select);
