...
export async function hasPaperModelRun({ modelId, rebalanceTime }) {
  if (!isSupabaseConfigured()) return false;
  const params = new URLSearchParams({
    select: "model_id",
    model_id: `eq.${modelId}`,
...
export async function fetchRecentPaperModelRuns(limit = 12) {
  if (!isSupabaseConfigured()) return [];
  const params = new URLSearchParams({
    select: "model_id,rebalance_time,data_cutoff_time,state,deployment_gate_passed,capital_weight,predicted_beta,gross_exposure,eligible_symbols,targets,diagnostics,created_at",
