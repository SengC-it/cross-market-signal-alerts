...
export async function updateSentAlertPayload(signalKey, payload) {
  if (!isSupabaseConfigured()) return;
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.sentAlerts}?signal_key=eq.${encodeURIComponent(signalKey)}`, {
    method: "PATCH",
    headers: {
      ...supabaseHeaders(),
      "Content-Type": "application/json",
...
export async function fetchSentAlertsForReview(limit = 200) {
  if (!isSupabaseConfigured()) return [];
  const params = sentAlertsForReviewParams(limit);
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.sentAlerts}?${params}`, {
    headers: supabaseHeaders()
  });
...
export function sentAlertsForReviewParams(limit = 200) {
  return new URLSearchParams({
    select: sentAlertSelect(),
    or: "(delivery_status.eq.sent,delivery_mode.eq.PAPER)",
    order: "sent_at.desc",
    limit: String(limit)
  });
}
...
export async function fetchRecentPaperEmailRuns(limit = 100) {
  ...
  if (!response.ok) throw new Error(`Supabase paper email history lookup failed: ${response.status} ${await response.text()}`);
  return response.json();
}
