...
export function classifyProductionSignal(signal = {}, {
  ...
  if (strategyId === DYNAMIC_STRENGTH
    && signal.signalVariant === STRONG_CORE_VARIANT) {
    return {
      tier: SIGNAL_TIERS.OBSERVATION,
      label: "STRONG CORE / OBSERVATION_ONLY",
      deliveryMode: "WEB",
      emailEligible: false,
      webRecorded: true,
      shadowRecorded: false,
      historicalSampleStatus: "INSUFFICIENT_EVIDENCE / 仅后台观察",
      reason: "STRONG_CORE_8_10 样本不足且 4h edge 未通过，不发送普通邮件，仅保留 Web/后台观察。"
    };
  }
  ...
  if (strategyId === DYNAMIC_STRENGTH) {
    return {
      tier: SIGNAL_TIERS.OBSERVATION,
      label: "STRONG OBSERVATION / WEB_ONLY",
      deliveryMode: "WEB",
      emailEligible: false,
      webRecorded: true,
      shadowRecorded: false,
      historicalSampleStatus: "INSUFFICIENT_EVIDENCE / LEGACY_OR_NON_EXTENSION / 仅后台观察",
      reason: "legacy/non-extension dynamic strength 仅后台观察，不发送普通邮件；只有 STRONG_EXTENSION_10_15 可进入观察邮件。"
    };
  }
